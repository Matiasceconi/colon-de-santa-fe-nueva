...
export default async function(req) {
  try {
    ...
    // Prioriza el nuevo DailyScheduleItem. Si todavía no fue sincronizado por el staff,
    // conserva DayEvent como fallback para no dejar al jugador sin agenda.
    let scheduleItems:any[] = [];
    if (squadId) {
      scheduleItems = await base44.asServiceRole.entities.DailyScheduleItem.filter(
        { squad_id: squadId, date: today },
        "start_time",
        200
      ).catch(() => []);
    }
    const visibleSchedule = scheduleItems.filter((item:any) => {
      if (item.status === 'cancelled' || item.visibility === 'staff_only') return false;
      if (item.visibility === 'specific_players') return (item.player_ids || []).includes(playerId);
      return true;
    });
    let schedule:any[] = visibleSchedule.map((item:any) => ({
      id: item.id,
      title: item.title,
      time: item.start_time || '',
      end_time: item.end_time || '',
      event_type: item.item_type || 'Actividad',
    ...
    if (!schedule.length && squadId) {
      const dayEvents = await base44.asServiceRole.entities.DayEvent.filter({ squad_id: squadId, date: today }, "start_time", 100).catch(() => []);
      schedule = dayEvents.map((e:any) => ({ id:e.id,title:e.title,time:e.start_time||e.time||'',end_time:e.end_time||'',event_type:e.event_type||e.type||'',item_type:'other',source_type:'day_event',location:e.location||'',duration_minutes:e.duration_minutes||null,notes:e.notes||'',rival:e.rival||'' }));
    }
    schedule.sort((a:any,b:any) => (a.time || '99:99').localeCompare(b.time || '99:99'));
