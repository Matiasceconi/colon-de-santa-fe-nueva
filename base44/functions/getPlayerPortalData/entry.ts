...
export default async function(req) {
  try {
    ...
    // Cronograma individual del día. Se filtra por visibilidad para que el jugador
    // no reciba actividades internas del staff ni tareas de otros futbolistas.
    const rawSchedule = await base44.asServiceRole.entities.DailyScheduleItem.filter(
    ...
    const schedule = rawSchedule
      ...
      .filter((item:any) => item.visibility !== 'staff_only')
      .filter((item:any) => item.visibility !== 'specific_players' || (item.player_ids || []).includes(playerId))
      .map((item:any) => ({
        id: item.id,
        title: item.title,
      ...
      .sort((a:any,b:any) => (a.start_time || '99:99').localeCompare(b.start_time || '99:99'));
