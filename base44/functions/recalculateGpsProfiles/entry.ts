...
Deno.serve(async (req) => {
  try {
    ...
    const [sessions, matches, canonicalMinutes, legacyMinutes, matchGpsRows, sessionGpsRows, sessionPlayers, memberships, referenceConfigs] = await Promise.all([
      ...
      base44.asServiceRole.entities.MinutesRecord.list('-match_date', 10000).catch(() => []),
      ...
      base44.asServiceRole.entities.GPSReferenceConfiguration.filter({ squad_id: squadId }, '-updated_at', 50).catch(() => []),
    ]);

    const referenceConfig = referenceConfigs.find((row) => !seasonId || !row.season_id || String(row.season_id) === String(seasonId)) || {};
    const minMatchMinutes = Math.max(1, Number(referenceConfig.min_match_minutes || 80));
    const competitionWindow = Math.max(1, Number(referenceConfig.competition_window || 5));
    const mdWindow = Math.max(1, Number(referenceConfig.md_window || 5));

    const canonicalKeys = new Set(canonicalMinutes.map((row) => `${row.match_id}:${row.player_id}`));
    const minutes = [
      ...canonicalMinutes.map((row) => ({ ...row, minutes: Number(row.minutes_played ?? row.minutes_calculated ?? 0), hidden_from_reports: false })),
      ...legacyMinutes.filter((row) => !canonicalKeys.has(`${row.match_id}:${row.player_id}`)),
    ];
    ...
    for (const playerId of playerIds) {
      const eligibleMatchIds = [...new Set(minutes
        .filter((m) => m.player_id === playerId && Number(m.minutes) >= minMatchMinutes && !m.hidden_from_reports && validMatchMap[m.match_id])
        .map((m) => m.match_id))]
        .sort((a, b) => (validMatchMap[b]?.date || '').localeCompare(validMatchMap[a]?.date || ''))
        .slice(0, competitionWindow);
      const eligibleMatchSet = new Set(eligibleMatchIds);

      const competitionRows = matchGpsRows.filter((r) => r.player_id === playerId && eligibleMatchSet.has(r.session_id));
      if (competitionRows.length) {
        const matchesWithGps = new Set(competitionRows.map((r) => r.session_id).filter(Boolean));
        await upsert(base44, 'PlayerCompetitionProfile', { player_id: playerId, squad_id: squadId, season_id: seasonId }, {
          matches_used: matchesWithGps.size,
          avg_total_distance: avgOf(competitionRows, 'total_distance'),
          avg_m_min: avgOf(competitionRows, 'meters_per_minute'),
          avg_distance_14_19_8: avgOf(competitionRows, 'distance_14_19'),
      ...
      for (const day of daysSet) {
        const allDayRows = playerSessionRows.filter((r) => {
          const session = sessionMap[r.session_id];
          const microDay = session?.microcycle_day || session?.match_day_code;
          const sessionPlayer = sessionPlayerMap[`${r.session_id}:${playerId}`];
          return session && microDay === day && isNormalGpsRow(r, sessionPlayer);
        });
        const recentSessionIds = [...new Set(allDayRows.map((r) => r.session_id))]
          .sort((a, b) => (sessionMap[b]?.date || '').localeCompare(sessionMap[a]?.date || ''))
          .slice(0, mdWindow);
        const recentSessionSet = new Set(recentSessionIds);
        const rows = allDayRows.filter((r) => recentSessionSet.has(r.session_id));
