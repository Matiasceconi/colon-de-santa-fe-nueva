...
Deno.serve(async (req) => {
  try {
    ...
    for (const squadId of squadIds) {
      const [sessionRows, referenceConfigs] = await Promise.all([
      ...
      const sessions = sessionRows.filter((s) => seasonMatches(s, seasonId));
      ...
      for (const g of groups) {
        ...
        for (const [md, allMdRows] of Object.entries(mdGroups)) {
          const recentSessionIds = [...new Set(allMdRows.map((r) => r.session_id))]
            .sort((a, b) => (sessionMap[b]?.date || '').localeCompare(sessionMap[a]?.date || ''))
            .slice(0, mdWindow);
          const recentSessionSet = new Set(recentSessionIds);
          const mdRows = allMdRows.filter((r) => recentSessionSet.has(r.session_id));
