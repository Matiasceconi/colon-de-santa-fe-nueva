...
export default async function(req) {
  try {
    ...
    if (!isPlatformAdmin) {
      const allowedSquadIds = staffAccess?.all_squads
        ? (await base44.asServiceRole.entities.Squad.list('name', 500)).map(squad => squad.id)
        : (staffAccess?.squad_ids || []);
      const currentSquadIds = Array.isArray(user.squad_ids) ? user.squad_ids : [];
      const accessChanged = currentSquadIds.length !== allowedSquadIds.length || currentSquadIds.some(id => !allowedSquadIds.includes(id));
      if (accessChanged) await base44.asServiceRole.entities.User.update(user.id, { squad_ids: allowedSquadIds });
    }
    ...
    return Response.json({
      ...
      staff_access: staffAccess ? {
        ...
        invitation_status: staffAccess.invitation_status || 'active',
        password_setup_required: !!staffAccess.password_setup_required,
