...
export default async function(req) {
  try {
    ...
    if (!planInput.is_template && assignmentsInput.length !== 1) {
      return Response.json({ error: 'Cada plan complementario debe pertenecer a un único jugador' }, { status: 400 });
    }
    const primaryAssignment = assignmentsInput[0] || null;
    ...
    const planPayload = {
      ...
      player_id: planInput.is_template ? '' : (primaryAssignment?.player_id || planInput.player_id || ''),
      player_name: planInput.is_template ? '' : (primaryAssignment?.player_name || planInput.player_name || ''),
      name: planInput.name,
