...
Deno.serve(async (req) => {
  try {
    ...
    const allowed = await requireModulePermission(base44, user, 'area_medica', 'can_edit');
    if (!allowed) return Response.json({ error: 'No tenés permiso para sincronizar el área médica.' }, { status: 403 });

    const profiles = await base44.asServiceRole.entities.InstitutionProfile.filter({ active: true }, 'created_date', 10);
    const institution = profiles[0];
    if (!institution) return Response.json({ error: 'No hay una institución activa configurada.' }, { status: 409 });
    const integrations = await base44.asServiceRole.entities.InstitutionIntegration.filter({ institution_id: institution.id, integration_key: 'medical_google_sheet', enabled: true }, '-updated_at', 10);
    const integration = integrations[0];
    const spreadsheetId = String(integration?.config?.spreadsheet_id || '').trim();
    const range = String(integration?.config?.range || 'A:J').trim() || 'A:J';
    const headerRows = Math.max(0, Number(integration?.config?.header_rows ?? 2));
    if (!spreadsheetId) return Response.json({ error: 'La integración médica con Google Sheets no está configurada para este club.', integration_not_configured: true }, { status: 409 });
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      const payload = {
        ...
        linked: isLinked,
        source_sheet_row_id: String(i + headerRows + 1),
      ...
      if (existing) {
        // Si un registro integrado fue corregido manualmente dentro de PerformancePitch,
        // la sincronización conserva ese ajuste y solo refresca metadatos de origen.
        if (existing.source === 'app' && existing.edited_at && existing.source_sheet_row_id) {
          await base44.asServiceRole.entities.MedicalEpisode.update(existing.id, {
            linked: isLinked,
            player_id: isLinked ? playerId : existing.player_id || '',
            last_synced_at: payload.last_synced_at,
            source_sheet_row_id: payload.source_sheet_row_id,
          });
        } else {
