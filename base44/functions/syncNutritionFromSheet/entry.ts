...
async function getWorkbook(base44, driveToken, metadata, fileId = SOURCE_FILE_ID) {
...
function stageUpsert(payload, keyField, byKey, byRow, seenIds, creates, updates) {
  ...
  if (existing) {
    // Un control integrado puede ser corregido manualmente dentro de PerformancePitch.
    // En ese caso la sincronización sigue verificando la fila de origen, pero no pisa
    // los valores nutricionales que el profesional decidió ajustar en la aplicación.
    if (existing.source === 'app' && existing.edited_at && existing.source_file_id === SOURCE_FILE_ID) {
      const syncMetadata = {
        id: existing.id,
        ...
        source_file_mime_type: payload.source_file_mime_type,
        row_hash: payload.row_hash,
        raw_values: payload.raw_values,
      };
      Object.keys(syncMetadata).forEach((field) => syncMetadata[field] === undefined && delete syncMetadata[field]);
      updates.push(syncMetadata);
      byKey[key] = { ...existing, ...syncMetadata };
    } else {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const nativeSpreadsheetId = existingState?.native_spreadsheet_id || '';
    const usingNativeSpreadsheet = Boolean(nativeSpreadsheetId);
    let driveAccessToken = '';
    let metadata;

    if (usingNativeSpreadsheet) {
      const sheets = await base44.asServiceRole.connectors.getConnection('googlesheets');
      const metadataResponse = await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/${nativeSpreadsheetId}?fields=properties.title`,
        { headers: { Authorization: `Bearer ${sheets.accessToken}` } },
      );
      if (!metadataResponse.ok) throw new Error(await metadataResponse.text());
      const sheetMetadata = await metadataResponse.json();
      metadata = {
        id: nativeSpreadsheetId,
        name: existingState?.source_file_name || sheetMetadata.properties?.title || 'Planilla de Nutrición',
        mimeType: SHEETS_MIME,
        modifiedTime: startedAt,
        webViewLink: existingState?.native_spreadsheet_url || `https://docs.google.com/spreadsheets/d/${nativeSpreadsheetId}/edit`,
      };
    } else {
    ...
    const workbook = await getWorkbook(base44, driveAccessToken, metadata, nativeSpreadsheetId || SOURCE_FILE_ID);
