...
export function classifyChange(
  ...
  if (!currentVsPrevious.hasPrevious || currentVsPrevious.changeAbs === null) vsPrevious = "no_previous";
  else if (currentVsPrevious.changeAbs === 0) vsPrevious = "neutral";
  else if (isBetter(currentVsPrevious.changeAbs)) vsPrevious = "improvement";
  else if (isWorse(currentVsPrevious.changeAbs)) vsPrevious = "decline";
  else vsPrevious = "neutral";

  let vsBaseline: "improvement" | "decline" | "neutral" | "no_baseline" | "insufficient";
  if (!currentVsBaseline.baselineSufficient || currentVsBaseline.changeAbs === null) vsBaseline = currentVsBaseline.baselineSufficient ? "no_baseline" : "insufficient";
  else if (currentVsBaseline.changeAbs === 0) vsBaseline = "neutral";
  else if (isBetter(currentVsBaseline.changeAbs)) vsBaseline = "improvement";
  else if (isWorse(currentVsBaseline.changeAbs)) vsBaseline = "decline";
  else vsBaseline = "neutral";

  const isMixed = (vsPrevious === "improvement" && vsBaseline === "decline") || (vsPrevious === "decline" && vsBaseline === "improvement");
  const isImprovement =
  ...
  return { vs_previous: vsPrevious, vs_baseline: vsBaseline, is_mixed: isMixed, is_improvement: isImprovement, is_decline: isDecline };
}
...
export function detectAsymmetrySignal(
  ...
  if (threshold === null || !isFinite(threshold)) return { flagged: false, reason: "Sin umbral de asimetría configurado" };
  if (asymmetryMagnitude > threshold) {
    return { flagged: true, reason: `Asimetría ${asymmetryMagnitude.toFixed(1)}% supera el umbral ${threshold}%` };
  }
...
export function detectAnomaly(
  ...
  if (baselineStd === 0 || !isFinite(baselineStd)) return { anomaly: false, reason: "" };
  const z = Math.abs((currentValue - baselineValue) / baselineStd);
  if (z >= 3) return { anomaly: true, reason: `Valor anómalo (z=${z.toFixed(2)})` };
