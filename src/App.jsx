...
import ClubAccessHome from '@/pages/ClubAccessHome';
...
import EquipmentOperations from '@/pages/EquipmentOperations';
import ClubDashboard from '@/pages/ClubDashboard';
import ClubIdentityAdmin from '@/pages/ClubIdentityAdmin';
import ValdDashboard from '@/pages/ValdDashboard';
...
import ActivateStaffAccount from '@/pages/ActivateStaffAccount';
...
function StaffLanding() {
  const { isAdmin, squads = [] } = useWorkspace();
  if (isAdmin && squads.length === 0) return <Navigate to="/setup" replace />;
      ...
      <PlayerCard360Provider>
        <Routes>
          <Route element={<Layout />}>
            <Route path="/club-dashboard" element={<StaffLanding />} />
            ...
            <Route path="/performance/kinesiology" element={<Kinesiology />} />
            ...
            <Route path="/club-operations/equipment" element={<EquipmentOperations />} />
            <Route path="/evaluations" element={<Evaluations />} />
            <Route path="/setup" element={<ClubSetupWizard />} />
            <Route path="/implementation-guide" element={<ImplementationGuide />} />
            <Route path="/player-guide" element={<PlayerGuide />} />
            <Route path="/roles-permissions" element={<RolesPermissions />} />
            <Route path="/competencias-afa" element={<CompetitionCenter />} />
          ...
          <Route path="/provisioning" element={<ProvisioningPortal />} />
...
function AppShell() {
  ...
  const isStaffActivation = searchParams.get("activate_staff") === "1";

  if (isLoadingPublicSettings || isLoadingAuth) return <LoadingScreen />;

  // Alta privada desde el correo de invitación. También funciona desde la raíz
  // para que Outlook y el hosting de Base44 no pierdan la pantalla de activación.
  if (pathname === "/activate-staff" || (pathname === "/" && isStaffActivation)) {
    return <ActivateStaffAccount />;
  }

  // Vista pública para comprobar el portal sin salir de la cuenta de preparación.
  if (pathname === "/portal-preview") return <ClubAccessHome />;

  // ── Rutas públicas (sin auth, sin WorkspaceProvider) ─────────────────────
  if (pathname === '/') {
    if (!isAuthenticated) return <ClubAccessHome />;
  ...
  if (pathname === '/register') {
    ...
    return <Navigate to="/activate-staff" replace />;
  }
  ...
  // Los administradores de plataforma ingresan al software operativo normalmente.
  // El portal de preparación queda disponible como herramienta en /provisioning.
