...
const TIMEZONES = ["America/Argentina/Buenos_Aires", "America/Montevideo", "America/Santiago", "America/Mexico_City", "America/Bogota", "America/Lima", "America/New_York", "Europe/Madrid", "Europe/London", "UTC"];
const LANGUAGES = ["Español", "English", "Português"];
const CURRENCIES = ["ARS", "USD", "EUR", "MXN", "UYU", "CLP", "BRL"];
const COLOR_FIELDS = ["brand_primary", "brand_secondary", "brand_accent", "brand_background", "brand_surface", "brand_text_primary", "brand_text_secondary"];
...
function ExportPreview({ form }) {
  const primary = isValidHex(form.brand_primary) ? form.brand_primary : "#1E293B";
  const landscape = form.default_orientation === "landscape";
  return (
    <div>
      <p className="mb-2 text-xs font-semibold text-zinc-400">Vista previa del encabezado de informe</p>
      <div className={`mx-auto overflow-hidden rounded-lg border border-zinc-700 bg-white text-zinc-900 shadow-xl ${landscape ? "max-w-xl aspect-[1.414/1]" : "max-w-sm aspect-[1/1.414]"}`}>
        <div className="flex items-center gap-3 border-b px-4 py-3" style={{ borderColor: primary }}>
          <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-lg bg-zinc-100">{form.shield_url ? <img src={form.shield_url} alt="" className="h-full w-full object-contain" /> : <Building2 size={18} className="text-zinc-400" />}</div>
          <div className="min-w-0 flex-1"><p className="truncate text-sm font-black">{form.official_name || "Club"}</p><p className="text-[9px] text-zinc-500">{form.show_squad_name ? "Plantel" : ""}{form.show_squad_name && form.show_season ? " · " : ""}{form.show_season ? (form.default_season || "Temporada") : ""}</p></div>
          {form.sponsor_logo_url && <img src={form.sponsor_logo_url} alt="" className="h-7 w-16 object-contain" />}
        </div>
        <div className="p-4"><div className="h-3 w-36 rounded" style={{ backgroundColor: primary }} /><div className="mt-4 grid grid-cols-3 gap-2">{[1,2,3].map((item) => <div key={item} className="rounded border border-zinc-200 p-2"><div className="h-2 w-10 rounded bg-zinc-300"/><div className="mt-2 h-4 w-14 rounded bg-zinc-100"/></div>)}</div><div className="mt-4 h-24 rounded border border-zinc-200 bg-zinc-50" /></div>
        <div className="mt-auto flex items-center justify-between border-t border-zinc-200 px-4 py-2 text-[8px] text-zinc-400"><span>{form.export_footer_text || "Informe PerformancePitch"}</span><span>{form.show_export_date ? "DD/MM/YYYY" : ""}{form.show_performancepitch_brand ? " · PerformancePitch" : ""}</span></div>
      </div>
    </div>
  );
}

export default function InstitutionSettingsPanel({ isAdmin, squads = [], visibleTabs = null, defaultTab = "info", titleMode = "full", seasonOptions = [], onSaved = null }) {
  const { toast } = useToast();
  const [profile, setProfile] = useState(null);
  const [form, setForm] = useState(EMPTY);
  const allowedTabs = useMemo(() => visibleTabs?.length ? TABS.filter((item) => visibleTabs.includes(item.id)) : TABS, [visibleTabs]);
  const initialTab = allowedTabs.some((item) => item.id === defaultTab) ? defaultTab : (allowedTabs[0]?.id || "info");
  ...
  useEffect(() => { load(); }, []);
  useEffect(() => {
    if (!allowedTabs.some((item) => item.id === tab)) setTab(allowedTabs[0]?.id || "info");
  }, [allowedTabs, tab]);
  ...
  async function save() {
    const invalidColor = COLOR_FIELDS.find((field) => form[field] && !isValidHex(form[field]));
    if (invalidColor) {
      toast({ title: "Colores inválidos", description: "Todos los colores configurados deben tener formato #RRGGBB", variant: "destructive" });
      return;
    }
    ...
    try {
      const cleaned = {
        ...form,
        official_name: String(form.official_name || "").trim(),
        short_name: String(form.short_name || "").trim(),
        abbreviation: String(form.abbreviation || "").trim().toUpperCase(),
        country: String(form.country || "").trim(),
        province: String(form.province || "").trim(),
        city: String(form.city || "").trim(),
        address: String(form.address || "").trim(),
        website: String(form.website || "").trim(),
        institutional_email: String(form.institutional_email || "").trim(),
        phone: String(form.phone || "").trim(),
        instagram: String(form.instagram || "").trim(),
        stadium: String(form.stadium || "").trim(),
        training_ground: String(form.training_ground || "").trim(),
        description: String(form.description || "").trim(),
        export_footer_text: (form.export_footer_text || "").trim(),
      };
      const updated = await updateInstitutionProfile(profile.id, cleaned);
      setProfile(updated);
      setForm((current) => ({ ...current, ...cleaned }));

      // PublicClubBrand es una proyección automática: no se edita como segunda fuente de identidad.
      const publicBrandPayload = {
        club_name: cleaned.official_name,
        short_name: cleaned.short_name || cleaned.official_name,
        logo_url: cleaned.shield_url || "",
        primary_color: cleaned.brand_primary,
        accent_color: cleaned.brand_accent,
        support_email: cleaned.institutional_email || "",
        active: true,
        updated_at: new Date().toISOString(),
      };
      const publicBrands = await base44.entities.PublicClubBrand.filter({ active: true }, "-updated_at", 1).catch(() => []);
      if (publicBrands?.[0]?.id) await base44.entities.PublicClubBrand.update(publicBrands[0].id, publicBrandPayload);
      else await base44.entities.PublicClubBrand.create(publicBrandPayload);

      if (onSaved) await onSaved();
      toast({ title: "Configuración del club guardada", description: "La identidad y las preferencias ya se aplicaron al entorno activo." });
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      {/* Encabezado */}
      {titleMode !== "compact" && <div className="flex items-start gap-4 rounded-xl border border-zinc-800 bg-zinc-900 p-4">
      ...
      {titleMode === "compact" && isAdmin && (
        <div className="flex justify-end">
          <button onClick={save} disabled={saving} className="flex items-center gap-1.5 rounded-lg border border-cyan-500/30 bg-cyan-500/15 px-4 py-2 text-sm font-semibold text-cyan-200 hover:bg-cyan-500/25 disabled:opacity-50">
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
        ...
        {tab === "prefs" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <Field label="Temporada predeterminada">{seasonOptions.length ? <select className={inputCls} value={form.default_season || ""} onChange={(e) => set("default_season", e.target.value)} disabled={!isAdmin}><option value="">—</option>{seasonOptions.map((season) => <option key={season} value={season}>{season}</option>)}</select> : <input className={inputCls} value={form.default_season || ""} onChange={(e) => set("default_season", e.target.value)} disabled={!isAdmin} />}</Field>
            <Field label="Idioma"><select className={inputCls} value={form.language || "Español"} onChange={(e) => set("language", e.target.value)} disabled={!isAdmin}>{[...new Set([...LANGUAGES, form.language].filter(Boolean))].map((language) => <option key={language} value={language}>{language}</option>)}</select></Field>
            <Field label="Zona horaria"><select className={inputCls} value={form.timezone || "America/Argentina/Buenos_Aires"} onChange={(e) => set("timezone", e.target.value)} disabled={!isAdmin}>{[...new Set([...TIMEZONES, form.timezone].filter(Boolean))].map((timezone) => <option key={timezone} value={timezone}>{timezone}</option>)}</select></Field>
            <Field label="Formato de fecha"><select className={inputCls} value={form.date_format || "DD/MM/YYYY"} onChange={(e) => set("date_format", e.target.value)} disabled={!isAdmin}><option value="DD/MM/YYYY">DD/MM/YYYY</option><option value="MM/DD/YYYY">MM/DD/YYYY</option><option value="YYYY-MM-DD">YYYY-MM-DD</option></select></Field>
            ...
            <Field label="Moneda"><select className={inputCls} value={form.currency || "ARS"} onChange={(e) => set("currency", e.target.value)} disabled={!isAdmin}>{[...new Set([...CURRENCIES, form.currency].filter(Boolean))].map((currency) => <option key={currency} value={currency}>{currency}</option>)}</select></Field>
          </div>
        )}
