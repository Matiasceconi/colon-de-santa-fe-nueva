...
function isCurrentClub(fixture, teamName) {
...
function FixtureRow({ fx, isYouth, teamName }) {
  ...
  const clubHome = fx.homeTeam === teamName;
  const clubAway = fx.awayTeam === teamName;
  ...
  return (
    <div>
      ...
      <div className="space-y-1.5">
        {shown.map((fx, i) => (
          <FixtureRow key={fx.id || i} fx={fx} isYouth={isYouth} teamName={teamName} />
        ))}
      </div>
    </div>
  );
...
function SeniorTab({ fixtures, loading, competitionName, defaultTournament, subtitle, teamName }) {
  const [tournament, setTournament] = useState(defaultTournament || "Clausura");

  const teamFixtures = useMemo(
    () => fixtures.filter(
      (f) => f.competitionName === competitionName && isCurrentClub(f, teamName) && (!f.tournament || f.tournament === tournament)
    ),
    [fixtures, competitionName, tournament, teamName]
  );
...
export default function FixturesSection() {
  ...
  const teamName = clubBrand?.name || "Club";
  const [fixtures, setFixtures] = useState([]);
  const [loading, setLoading] = useState(true);
