...
function ClubBrandShield({ size = "w-8 h-8" }) {
...
// Calcula resultado desde la perspectiva del club
function clubResult(fixture) {
  if (fixture.status !== "played" || fixture.homeScore == null || fixture.awayScore == null) return null;
  const clubGoals = fixture.isHome ? fixture.homeScore : fixture.awayScore;
...
function CategorySummary({ fixtures }) {
  const stats = useMemo(() => {
    ...
    played.forEach((f) => {
      ...
      if (!s) return;
      const clubGoals = f.isHome ? f.homeScore : f.awayScore;
      const rivalGoals = f.isHome ? f.awayScore : f.homeScore;
      s.PJ++;
      s.GF += clubGoals || 0;
      s.GC += rivalGoals || 0;
      if (clubGoals > rivalGoals) s.PG++;
      else if (clubGoals < rivalGoals) s.PP++;
      else s.PE++;
    });
            ...
            return (
              <div key={cat} className="flex items-center gap-3 p-2 rounded-lg bg-zinc-900/60">
                ...
                <span className="text-sm text-white font-medium flex-1">
                  {f.isHome ? `${clubName} ${clubGoals ?? "-"} - ${rivalGoals ?? "-"} ${f.awayTeam}` : `${f.homeTeam} ${f.homeScore ?? "-"} - ${f.awayScore ?? "-"} ${clubName}`}
                </span>
...
function NextRoundCard({ round, fixturesByCat }) {
  ...
  return (
    <div className="bg-gradient-to-br from-emerald-600/10 via-zinc-900 to-zinc-900 border border-emerald-500/30 rounded-2xl p-5">
      ...
      <p className="text-center text-sm font-semibold text-white mb-3">{isHome ? `${clubName} vs ${rival}` : `${rival} vs ${clubName}`}</p>
