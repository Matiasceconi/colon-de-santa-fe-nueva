  ...
  return (
    <div className="space-y-4">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-700 via-emerald-700 to-emerald-950 p-5 text-white">
        {clubBrand?.logoUrl && <img src={clubBrand.logoUrl} alt="" className="absolute right-4 top-4 h-20 w-20 object-contain opacity-25" />}
      ...
      {latest && config.showKpis && (
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">{KPI_KEYS.map((key) => { const def = REPORT_METRICS.find((item) => item.key === key); const delta = pctVs(latest.gpsRow?.[key], profileValue(key)); return <div key={key} className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-4"><p className="text-xs font-semibold text-zinc-400">{def.label}</p><p className="mt-2 text-3xl font-black tracking-tight text-white">{fmtMetric(latest.gpsRow?.[key], def.decimals)} <span className="text-xs font-semibold text-zinc-500">{def.unit}</span></p>{delta != null && <p className={`mt-1 text-[10px] font-bold ${tone(delta)}`}>{delta > 0 ? "+" : ""}{delta}% vs perfil</p>}</div>; })}</div>
        </div>
      )}
