...
export default function FormationPoster({ containerRef, fieldRef, match, system, positions, playerMap, suplentes = [], captainPlayerId, onDrop, onPlayerClick, onCaptainChange }) {
  ...
  const clubShort = clubBrand?.shortName || "CLUB";
  ...
  return (
    <div ref={containerRef} className="overflow-hidden rounded-[30px] border border-yellow-400/20 bg-gradient-to-r from-[#002b06] via-[#003b08] to-[#3a3c17] p-6 text-white shadow-2xl">
      <div className="relative pb-5 text-center">
        {clubLogo ? (
          <img src={clubLogo} alt={clubName} className="absolute left-0 top-0 h-24 w-24 object-contain" />
        ) : (
          <div className="absolute left-0 top-0 h-24 w-24 rounded-2xl bg-gradient-to-br from-blue-500 to-blue-700 border border-blue-400/30 flex items-center justify-center text-white font-black text-3xl">{clubShort}</div>
        )}
        ...
        <div className="mt-5 text-yellow-300">
          <h3 className="text-2xl font-black tracking-tight">{clubName.toUpperCase()} vs {String(rival).toUpperCase()}</h3>
