...
function getTournament(match) {
  ...
  const normalized = competition.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  if (normalized.includes("apertura")) return "Proyección Apertura";
  if (normalized.includes("clausura")) return "Clausura";
  if (normalized.includes("copa argentina")) return "Copa Argentina";
  if (normalized.includes("juven")) return "Juveniles";
  if (normalized.includes("amist")) return "Amistosos";
  if (normalized.includes("regional")) return "Torneo Regional";
  return "Otro";
}
...
export default function MinutosTab({ match, players = [], onRegisterSave, onMatchUpdated, refreshKey = 0 }) {
  ...
  async function loadMinutes() {
    ...
    try {
      ...
      const durationIsOfficial = Boolean(match.total_duration_minutes && !["calculated_from_max_player", "gps_suggestion"].includes(match.duration_source));
      const suggestedDuration = match.gps_duration_suggestion || (minuteRows || []).find((row) => row.gps_suggested_minutes != null || row.source === "gps_csv")?.match_duration_minutes;
      setDuration(String(durationIsOfficial ? match.total_duration_minutes : suggestedDuration || 90));
      ...
      callupPlayers.forEach(({ player, callup }) => {
        ...
        const gpsOnly = Boolean(record && record.source === "gps_csv" && record.official_confirmed !== true && record.manual_override !== true);
        const recordOfficial = Boolean(record && !gpsOnly);
        const legacy = gpsOnly ? null : legacyMap[player.id];
        const isStarter = recordOfficial ? !!record.started : legacy ? !!legacy.is_starter : callup.lineup_role === "titular";
        const entered = isStarter ? true : recordOfficial ? !!record.entered : legacy ? Number(legacy.minutes || 0) > 0 : false;
        const gpsMinutes = record?.gps_suggested_minutes ?? (gpsOnly ? record?.minutes_played : null);
        nextEntries[player.id] = {
          is_starter: isStarter,
          entered,
          in_minute: isStarter ? "" : (recordOfficial ? (record?.entered_minute_label || (record?.entered_minute != null ? String(record.entered_minute) : "")) : legacy?.sub_in_minute != null ? String(legacy.sub_in_minute) : ""),
          out_minute: recordOfficial ? (record?.exit_minute_label || (record?.exit_minute != null ? String(record.exit_minute) : "")) : "",
          manual_override: recordOfficial ? !!record?.manual_override : false,
          manual_minutes: recordOfficial ? (record?.manual_override ? record.minutes_played : record?.minutes_played ?? legacy?.minutes ?? 0) : 0,
          manual_reason: recordOfficial ? (record?.manual_reason || "") : "",
          gps_minutes: gpsMinutes != null ? Number(gpsMinutes) : null,
          official_confirmed: recordOfficial,
        };
      });
  ...
  const rows = useMemo(() => calledPlayers.map((player) => {
    const entry = entries[player.id] || { is_starter: false, entered: false, in_minute: "", out_minute: "", manual_override: false, manual_minutes: 0, manual_reason: "", gps_minutes: null };
  ...
  const gpsSuggestionCount = rows.filter((row) => Number(row.entry.gps_minutes || 0) > 0).length;
  const officialCount = rows.filter((row) => row.entry.official_confirmed).length;
  const statusText = validation.errors.length === 0 && totalDuration > 0 ? (officialCount === rows.length ? "Minutos confirmados" : "Listo para confirmar") : "Pendientes";
  ...
  function applyGpsSuggestions() {
    const next = { ...entries };
    let applied = 0;
    calledPlayers.forEach((player) => {
      const current = next[player.id];
      const gps = Number(current?.gps_minutes || 0);
      if (!current || gps <= 0 || current.official_confirmed) return;
      next[player.id] = {
        ...current,
        entered: current.is_starter ? true : true,
        manual_override: true,
        manual_minutes: gps,
        manual_reason: current.manual_reason || "Confirmado desde sugerencia GPS",
      };
      applied++;
    });
    if (!applied) return toast({ title: "No hay sugerencias GPS pendientes" });
    setEntries(next);
    setDirty(true);
    toast({ title: `${applied} sugerencias aplicadas`, description: "Revisalas y presioná Guardar todo para convertirlas en minutos oficiales." });
  }
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const basePayload = {
          ...
          source: row.entry.manual_override ? "manual_override" : "match_minutes_tab",
          data_source: "match_minutes_tab",
          official_confirmed: true,
          gps_suggested_minutes: row.entry.gps_minutes != null ? Number(row.entry.gps_minutes) : null,
          gps_suggestion_source: row.entry.gps_minutes != null ? "gps_csv" : null,
      ...
      setEntries((current) => Object.fromEntries(Object.entries(current).map(([playerId, entry]) => [playerId, { ...entry, official_confirmed: true }])));
      setDirty(false);
      toast({ title: "Minutos oficiales guardados correctamente" });
    } catch (error) {
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2">{gpsSuggestionCount > 0 && <button onClick={applyGpsSuggestions} className="rounded-lg border border-sky-500/30 bg-sky-500/10 px-3 py-2 text-xs text-sky-300 transition hover:bg-sky-500/20"><Clock3 size={13} className="mr-1 inline" /> Aplicar {gpsSuggestionCount} sugerencias GPS</button>}<button onClick={setFullDurationToStarters} className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-300 transition hover:bg-emerald-500/20"><ShieldCheck size={13} className="mr-1 inline" /> Duración completa a titulares</button><button onClick={markSubsWithoutEntry} className="rounded-lg border border-blue-500/30 bg-blue-500/10 px-3 py-2 text-xs text-blue-300 transition hover:bg-blue-500/20"><UserPlus size={13} className="mr-1 inline" /> Marcar suplentes sin ingreso</button><button onClick={saveAll} disabled={!dirty || saving} className="rounded-lg bg-yellow-500 px-3 py-2 text-xs font-semibold text-zinc-950 transition hover:bg-yellow-400 disabled:cursor-not-allowed disabled:opacity-50"><Save size={13} className="mr-1 inline" /> {saving ? "Guardando..." : "Confirmar y guardar"}</button></div>
        </div>
        <div className="mt-3 flex flex-wrap gap-2 text-xs text-zinc-500"><span className="rounded-full border border-zinc-700 bg-zinc-950 px-3 py-1">{calledPlayers.length} convocados</span><span className="rounded-full border border-zinc-700 bg-zinc-950 px-3 py-1">{starterCount} titulares</span><span className="rounded-full border border-zinc-700 bg-zinc-950 px-3 py-1">{enteredSubs} suplentes ingresaron</span><span className="rounded-full border border-zinc-700 bg-zinc-950 px-3 py-1">{noEntrySubs} no ingresaron</span>{gpsSuggestionCount > 0 && <span className="rounded-full border border-sky-500/30 bg-sky-500/10 px-3 py-1 text-sky-300">{gpsSuggestionCount} sugerencias GPS</span>}<span className="rounded-full border border-zinc-700 bg-zinc-950 px-3 py-1">Estado: {statusText}</span></div>
        {(!match.total_duration_minutes || ["calculated_from_max_player", "gps_suggestion"].includes(match.duration_source)) && (match.gps_duration_suggestion || match.total_duration_minutes) && <div className="mt-3 rounded-xl border border-sky-500/30 bg-sky-500/10 p-3 text-sm text-sky-200"><Clock3 size={14} className="mr-1 inline" /> El GPS sugiere una duración de <strong>{match.gps_duration_suggestion || match.total_duration_minutes}'</strong>. Revisala y guardá esta pestaña para confirmarla como duración oficial.</div>}
...
function validateRows(rows, duration, starterCount) {
  ...
  rows.forEach(({ player, entry, minutes }) => {
    ...
    if (!entry.is_starter && entry.entered && !entry.manual_override && (input == null || Number.isNaN(input))) errors.push(`${name}: cargá un minuto de ingreso válido.`);
