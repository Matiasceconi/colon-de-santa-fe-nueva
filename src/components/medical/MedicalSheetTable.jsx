...
import MedicalEpisodeCreateModal from "./MedicalEpisodeCreateModal";
import { STATUS_LABELS, STATUS_BADGE, getRowColorClasses } from "./medicalStatusConfig";
import { dedupeMedicalEpisodes } from "./medicalDeduplication";
...
export default function MedicalSheetTable({ squadPlayerIds }) {
  ...
  const canCreate = can("create", "/performance/medical");
  const canEdit = can("edit", "/performance/medical");
  ...
  const uniqueEpisodes = useMemo(() => dedupeMedicalEpisodes(episodes).episodes, [episodes]);
  const baseFiltered = squadPlayerIds instanceof Set
    ? uniqueEpisodes.filter((e) => !e.player_id || squadPlayerIds.has(e.player_id))
    : uniqueEpisodes;

  const visiblePlayers = useMemo(() => squadPlayerIds instanceof Set ? players.filter((player) => squadPlayerIds.has(player.id)) : players, [players, squadPlayerIds]);
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          {canCreate && <button onClick={() => setCreating(true)} className="flex items-center gap-1.5 px-3 py-2 bg-white hover:bg-zinc-200 rounded-lg text-xs font-semibold text-zinc-900 shrink-0"><Plus size={13} /> Nuevo registro manual</button>}
          {canEdit && <button
            onClick={handleSync}
            disabled={syncing}
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {filtered.map((e) => {
                ...
                return (
                  <tr key={e.id} className={`border-b border-zinc-800/60 ${getRowColorClasses(e.medical_status)} ${canEdit ? "cursor-pointer hover:brightness-125" : ""}`} onClick={() => canEdit && setEditing(e)}>
                    ...
                    <td className="p-2.5 text-zinc-500 whitespace-nowrap">
                      <div>{e.source === "app" ? (e.source_sheet_row_id ? "Integrado + ajuste manual" : "Manual") : "Google Sheets"}</div>
                    ...
                    <td className="p-2.5">
                      {canEdit && <Pencil size={13} className="text-zinc-600" />}
                    </td>
                  </tr>
                );
              })}
      ...
      {creating && <MedicalEpisodeCreateModal players={visiblePlayers} onClose={() => setCreating(false)} onSaved={() => { setCreating(false); load(); }} />}
