...
export default function NutritionDashboard({ players, assessments, interpretations, activeSquad }) {
  ...
  const readingRecords = useMemo(
    () => interpretations.filter((row) => row.player_id && row.found_in_last_sync !== false && (row.source_sheet_name === "Informe de lectura" || row.source_sheet_name === "Carga manual")),
    [interpretations]
  );
  const skinfoldRecords = useMemo(
    () => assessments.filter((row) => row.player_id && row.found_in_last_sync !== false && row.sumatoria_6p != null && (row.source_sheet_name === "Seguimiento de pliegues" || row.tipo_medicion === "Seguimiento de pliegues" || row.source === "app")),
    [assessments]
  );
  const weightRecords = useMemo(
    () => assessments.filter((row) => row.player_id && row.found_in_last_sync !== false && row.peso != null && (row.source_sheet_name === "Seguimiento de peso" || row.tipo_medicion === "Seguimiento de peso" || row.source === "app")),
    [assessments]
  );
  ...
  function handleExport() {
    try {
    assertExportAllowed(can,"/performance/nutrition");
    ...
    XLSX.utils.book_append_sheet(workbook,XLSX.utils.aoa_to_sheet([["Club",clubBrand?.name||"Club"],["Plantel",activeSquad?.name||"Plantel"],["Informe",tabLabel],["Estado",statusFilter],["Búsqueda",query],["Generado",moment().format("YYYY-MM-DD HH:mm")],["Registros",output.length]]),"Contexto");
    XLSX.writeFile(workbook,buildExportFileName("Nutricion",[clubBrand?.name,activeSquad?.name,activeTab,moment().format("YYYYMMDD-HHmm")],"xlsx"));
    }catch(error){toast({title:error.message||"No se pudo exportar",variant:"destructive"});}
  }
