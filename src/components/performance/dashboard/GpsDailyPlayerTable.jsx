...
export default function GpsDailyPlayerTable({ gpsSources = [], selectedSourceIds = [], playerMap = {}, microcycleProfiles = [], competitionProfiles = [], squadId, season, playerIds = [] }) {
  ...
  const rows = useMemo(() => aggregateRows(activeItems, gpsByItem, playerMap, metrics, selectedSources.length, viewMode, playerIds), [activeItems, gpsByItem, playerMap, metrics, selectedSources.length, viewMode, playerIds]);
...
function aggregateRows(items, gpsByItem, playerMap, metrics, selectedSourceCount, viewMode, playerIds = []) {
  const map = {};
  items.forEach((item) => {
    (gpsByItem[item.id] || []).filter(validRow).filter((row) => !playerIds.length || playerIds.includes(row.player_id)).forEach((r) => {
