...
const SOURCE_TYPE_OPTIONS = [["training", "Entrenamiento"], ["match", "Partido"]];
...
export function getMicrocycleFilterLabels(filters, { players = [], gpsSources = [], metrics = [] }) {
  const labels = [];
  const player = players.find((p) => p.id === filters.playerId);
  const source = gpsSources.find((s) => s.id === filters.sourceId);
  ...
  if (filters.playerIds?.length) {
    const selectedNames = filters.playerIds.map((id) => players.find((p) => p.id === id)?.full_name).filter(Boolean);
    labels.push(selectedNames.length <= 2 ? selectedNames.join(" · ") : `${selectedNames.length} jugadores`);
  }
  ...
  if (filters.sessionType) labels.push(filters.sessionType === "Partido" ? "Partido" : "Entrenamiento");
  ...
  if (source) labels.push(source.title || "Carga");
...
export default function GpsMicrocycleFiltersPanel({ filters, onApply, players, gpsSources = [], metrics }) {
  ...
  const sourceDates = useMemo(() => [...new Set(gpsSources.map((s) => s.date).filter(Boolean))].sort(), [gpsSources]);
  const objectives = useMemo(() => [...new Set(gpsSources.map((s) => s.objective).filter(Boolean))].sort(), [gpsSources]);
  const squads = useMemo(() => [...new Map(gpsSources.filter((s) => s.squadId).map((s) => [s.squadId, s.squadId])).entries()], [gpsSources]);
  const seasons = useMemo(() => [...new Set(gpsSources.map((s) => s.seasonId).filter(Boolean))].sort().reverse(), [gpsSources]);
  const sortedSources = useMemo(() => [...gpsSources].sort((a, b) => (b.date || "").localeCompare(a.date || "")), [gpsSources]);
