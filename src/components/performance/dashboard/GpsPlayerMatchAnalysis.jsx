...
export default function GpsPlayerMatchAnalysis({
  ...
  useEffect(() => {
    ...
    base44.entities.MatchPlayerMinutes.filter({ player_id: player.id }, "-match_date", 500)
      ...
      .catch(() => { if (!cancelled) { setMatchOptions([]); setMessage("No se pudieron cargar los minutos. Volvé a seleccionar el jugador para reintentar."); } })
  ...
  async function publish() {
    ...
    setMessage("");
    try {
      const saved = await base44.functions.invoke("managePlayerMatchReport", { operation: "save", id: savedReportId, payload: payload() });
      const savedResult = saved.data || saved;
      if (savedResult.error) throw new Error(savedResult.error);
      const reportId = savedResult.report?.id;
      if (!reportId) throw new Error("No se pudo guardar el informe antes de publicar.");
  ...
  async function downloadPdf() {
    setBusy("pdf");
    try {
      await exportMatchReportPdf({ reportData, reportMeta: { title: title(), squadName }, staffComment, reportConfig, clubBrand });
    } finally {
      setBusy("");
    }
  }
  ...
  function downloadExcel() {
    setBusy("excel");
    try {
      exportMatchReportExcel({ reportData, reportMeta: { title: title(), squadName }, staffComment, reportConfig, clubBrand });
    } finally {
      setBusy("");
    }
  }
