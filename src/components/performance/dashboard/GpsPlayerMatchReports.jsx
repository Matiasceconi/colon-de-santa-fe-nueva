...
export default function GpsPlayerMatchReports({ playerId, squadId }) {
  ...
  async function download(report) {
    ...
    try {
      ...
      await exportMatchReportPdf({ reportData: data, reportMeta: { title: report.title }, staffComment: report.staff_comment, reportConfig: data.reportConfig, clubBrand });
    } finally {
      setBusy("");
    }
  }
  ...
  async function downloadExcel(report) {
    ...
    try {
      const data = await dataFor(report);
      exportMatchReportExcel({ reportData: data, reportMeta: { title: report.title }, staffComment: report.staff_comment, reportConfig: data.reportConfig, clubBrand });
    } finally {
      setBusy("");
    }
  }
