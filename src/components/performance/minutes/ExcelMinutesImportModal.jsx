          ...
          {simulation && (
            <>
              ...
              <div className="sticky bottom-0 flex flex-wrap justify-end gap-2 border-t border-zinc-800 bg-zinc-950/95 py-4 backdrop-blur">
                ...
                <button onClick={confirmImport} disabled={applying || !plan || !summary || summary.partidosDetectados <= 0 || summary.partidosPendientes > 0 || summary.jugadoresPendientes > 0} className="rounded-xl bg-yellow-500 px-4 py-2 text-sm font-semibold text-zinc-950 hover:bg-yellow-400 disabled:cursor-not-allowed disabled:opacity-50">{applying ? "Importando…" : "Confirmar importación"}</button>
              </div>
            </>
          )}
        </div>
