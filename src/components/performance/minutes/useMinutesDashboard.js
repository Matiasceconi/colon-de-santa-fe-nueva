...
function buildCallupsByMatch(rows) {
  return (rows || []).reduce((acc, row) => {
    if (!row.match_id || row.status === "desconvocado" || row.callup_status === "desconvocado") return acc;
    acc[row.match_id] = acc[row.match_id] || [];
    acc[row.match_id].push(row);
    return acc;
  }, {});
...
function dedupeCallups(rows) {
  ...
  return [...(rows || [])]
    ...
    .filter((row) => {
      const key = row.player_id || `name:${normalizeText(row.player_name)}`;
...
function isOfficialMinuteRow(row) {
  if (!row) return false;
  if (row.official_confirmed === true || row.manual_override === true) return true;
  if (row.source === "gps_csv" || row.data_source === "gps_csv") return false;
  // Registros históricos anteriores al flag official_confirmed se consideran oficiales
  // salvo que su fuente sea explícitamente GPS.
  return row.official_confirmed !== false;
}
...
function getMatchStatus(match) {
  ...
  const rows = dedupeMinuteRows(match.minuteRows || []).filter(isOfficialMinuteRow);
  const callups = dedupeCallups(match.callups || []);
  ...
  const recordPlayerIds = new Set(rows.map((row) => row.player_id).filter(Boolean));
  const missingCalledPlayers = callups.filter((row) => row.player_id && !recordPlayerIds.has(row.player_id));
  const finished = match.finished;
  const complete = finished && duration > 0 && callups.length > 0 && rows.length >= callups.length && missingCalledPlayers.length === 0 && incompleteRows.length === 0;
  ...
  if (callups.length === 0) return { label: "Revisar convocatoria", tone: "text-amber-300", badge: "border-amber-500/30 bg-amber-500/10" };
  return { label: `Pendiente ${missingCalledPlayers.length || Math.max(0, callups.length - rows.length)}`, tone: "text-yellow-300", badge: "border-yellow-500/30 bg-yellow-500/10" };
}
...
function buildPlayerPool(players, memberships, squadId) {
  ...
  const memberIds = new Set((memberships || [])
    .filter((membership) => membership.squad_id === squadId)
    .filter((membership) => String(membership.status || "activo") === "activo")
    .filter((membership) => !membership.effective_from || membership.effective_from <= today)
    .filter((membership) => !membership.effective_to || membership.effective_to >= today)
  ...
  return (players || []).filter((player) => player.squad_id === squadId && player.active !== false);
}
...
export default function useMinutesDashboard() {
  ...
  const [data, setData] = useState({ players: [], memberships: [], matches: [], minuteRows: [], callups: [], youthMinuteRows: [], competitions: [], aliases: [] });

  const loadData = useCallback(async () => {
    setLoading(true);
    const [minuteRows, callups, matches, players, competitions, aliases, memberships, youthMinuteRows] = await Promise.all([
      base44.entities.MatchPlayerMinutes.list("-updated_date", 5000).catch(() => []),
      base44.entities.MatchCallup.list("-updated_date", 5000).catch(() => []),
      ...
      base44.entities.SquadMembership.list("player_name", 3000).catch(() => []),
      listYouthMinutes().catch(() => []),
    ]);
    setData({ players, memberships, matches, minuteRows, callups, youthMinuteRows, competitions, aliases });
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubCallups = base44.entities.MatchCallup.subscribe(reload);
  ...
  const callupsByMatch = useMemo(() => buildCallupsByMatch(data.callups), [data.callups]);
  ...
  const normalizedMatches = useMemo(() => {
    return (data.matches || [])
      ...
      .map((match) => {
        ...
        const minuteRows = dedupeMinuteRows(minuteRowsByMatch[match.id] || []).filter(isOfficialMinuteRow);
        const callups = dedupeCallups(callupsByMatch[match.id] || []);
        const status = getMatchStatus({ ...match, finished, minuteRows, callups });
        return {
          ...match,
          resolvedCompetition,
  ...
  }, [data.matches, competitionsById, aliasMap, minuteRowsByMatch, callupsByMatch]);
  ...
  const playerRows = useMemo(() => {
    ...
    const rowsFromRecords = filteredMinuteRows
      ...
      .reduce((acc, row) => {
        ...
        const rows = rowsByPlayer[key] || [];
        const player = (data.players || []).find((candidate) => candidate.id === row.player_id);
        const availableMinutes = filteredFinishedMatches.reduce((sum, match) => sum + getMatchDuration(match), 0);
        acc[key] = {
  ...
  }, [filteredMinuteRows, playerPool, filteredFinishedMatches, filters.search, filters.sortBy, data.players]);

  const manualPlayers = useMemo(() => (playerPool || []).map((player) => ({
    player_id: player.id,
    player_name: player.full_name || `${player.first_name || ""} ${player.last_name || ""}`.trim(),
    position: player.position || "—",
    photo_url: player.photo_url || "",
  })).sort((a, b) => String(a.player_name).localeCompare(String(b.player_name))), [playerPool]);
