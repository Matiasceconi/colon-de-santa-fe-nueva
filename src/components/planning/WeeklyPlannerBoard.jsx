...
function MicrocycleExportView({ days, meta, dayLoads, summary, sessionDetails, sessionsById, sessionLibrary, calendarEvents, physicalObjectives, showChart, onExit }) {
  ...
  return <div className="min-h-screen bg-white text-zinc-950 p-6 print:p-0">
    ...
    <div className="rounded-2xl border-b-4 pb-4 mb-5 p-4 flex justify-between items-start" style={{ borderColor: clubBrand.colors.green, background: `linear-gradient(135deg, ${clubBrand.colors.panel}, #ffffff 62%, ${clubBrand.colors.yellow}22)` }}>
      <div className="flex items-center gap-4">
        <img src={clubBrand.logoUrl} alt={clubBrand.name} className="w-16 h-16 object-contain" />
        <div><p className="text-xs font-bold uppercase" style={{ color: clubBrand.colors.greenDark }}>{clubBrand.name}</p><h1 className="text-3xl font-black">Planificación de Microciclo</h1><p className="text-sm text-zinc-600">Semana {meta.week_number} · {meta.range_label}</p></div>
      </div>
      <div className="text-right text-sm"><p className="font-black uppercase" style={{ color: clubBrand.colors.greenDeep }}>{meta.week_type}</p><p>{meta.next_match}</p></div>
    </div>
    ...
    <div className={showChart ? "grid grid-cols-3 gap-3" : "grid grid-cols-1 gap-3"}><div className="border rounded-xl p-3" style={{ borderColor: clubBrand.colors.line }}><p className="text-xs font-black uppercase" style={{ color: clubBrand.colors.greenDark }}>Resumen automático</p><p className="text-sm mt-2 whitespace-pre-wrap">{summary}</p></div>{showChart && <div className="border rounded-xl p-3 col-span-2" style={{ borderColor: clubBrand.colors.line }}><LoadChart data={dayLoads} type="barras" clean /></div>}</div>
  </div>;
}
...
function LoadChart({ data, type, clean = false }) {
  ...
  if (type === "radar") return <ResponsiveContainer width="100%" height={260}><RadarChart data={chartData}><PolarGrid stroke={clubBrand.colors.line} /><PolarAngleAxis dataKey="day" tick={{ fill: text, fontSize: 10 }} /><Radar dataKey="Carga" stroke={clubBrand.colors.green} fill={clubBrand.colors.green} fillOpacity={0.35} /></RadarChart></ResponsiveContainer>;
  ...
  if (type === "linea") return <ResponsiveContainer width="100%" height={260}><LineChart data={chartData}>{common}<Line type="monotone" dataKey="Carga" stroke={clubBrand.colors.green} strokeWidth={3} /></LineChart></ResponsiveContainer>;
  if (type === "area") return <ResponsiveContainer width="100%" height={260}><AreaChart data={chartData}>{common}<Area type="monotone" dataKey="Carga" stroke={clubBrand.colors.green} fill={clubBrand.colors.green} fillOpacity={0.28} /></AreaChart></ResponsiveContainer>;
  return <ResponsiveContainer width="100%" height={260}><BarChart data={chartData}>{common}<Bar dataKey="Carga" fill={clubBrand.colors.green} radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer>;
}

export default function WeeklyPlannerBoard() {
  const { toast } = useToast();
  const { activeSquadId, activeSquad, activeSeasonId, clubBrand } = useWorkspace();
  ...
  return <div className="min-h-screen w-full min-w-0 text-zinc-950 space-y-3" style={{ background: clubBrand.colors.panel }}>
    ...
    <header className="border rounded-xl shadow-sm px-4 py-3 flex items-center justify-between gap-3 flex-wrap overflow-hidden" style={{ borderColor: clubBrand.colors.line, background: `linear-gradient(135deg, ${clubBrand.colors.white} 0%, ${clubBrand.colors.panel} 72%, ${clubBrand.colors.yellow}26 100%)` }}>
      <div className="flex items-center gap-3">
        <div className="w-12 h-12 rounded-xl bg-white border flex items-center justify-center shadow-sm p-1.5" style={{ borderColor: clubBrand.colors.green }}><img src={clubBrand.logoUrl} alt={clubBrand.name} className="w-full h-full object-contain" /></div>
        <div><p className="text-[10px] font-black uppercase tracking-widest" style={{ color: clubBrand.colors.greenDark }}>{clubBrand.name}</p><h1 className="text-lg font-black tracking-tight">PLANIFICADOR DE MICROCICLO</h1><p className="text-xs text-zinc-500">{activeSquad?.name || "Plantel"} · {activeSeasonId || activeSquad?.season || "Temporada"}</p></div>
      </div>
      <div className="flex gap-2 flex-wrap items-center">
        ...
        <button onClick={() => aiInputRef.current?.click()} className="px-3 py-2 rounded-lg text-white text-xs font-black flex items-center gap-2 shadow-sm" style={{ backgroundColor: clubBrand.colors.green }}><Sparkles size={14} /> {aiLoading ? "Creando..." : "Crear con IA"}</button>
        ...
        <button onClick={save} disabled={saving} className="px-3 py-2 rounded-lg text-xs font-black flex items-center gap-2 shadow-sm" style={{ backgroundColor: clubBrand.colors.yellow, color: clubBrand.colors.greenDeep }}><Save size={14} /> {saving ? "Guardando..." : "Guardar"}</button>
        <button onClick={() => setExportPromptOpen(true)} className="px-3 py-2 rounded-lg text-white text-xs font-black flex items-center gap-2 shadow-sm" style={{ backgroundColor: clubBrand.colors.greenDark }}><Download size={14} /> Exportar / Compartir</button>
      </div>
    </header>
