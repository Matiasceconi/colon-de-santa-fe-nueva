...
export default function PlayerCard360Header({
  ...
  const positionLabels = [...new Set([...(positionOptions.length ? positionOptions.map((item) => item.label) : DEFAULT_POSITION_OPTIONS), editForm?.position].filter(Boolean))];
      ...
      {editing && (
        <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
          <div>
            ...
            <select value={editForm.position || ""} onChange={e => { const position = e.target.value; const group = positionGroupFromOption(position, positionOptions); setEditForm(f => ({ ...f, position, position_group: group || f.position_group || "", player_type: group === "Arqueros" ? "arquero" : "jugador_campo" })); }}
          ...
          <div>
            ...
            <div className="mt-1 rounded-lg border border-zinc-700 bg-zinc-900 px-2 py-1.5 text-xs text-zinc-300">{editForm.position_group || positionGroupFromOption(editForm.position, positionOptions) || "Sin grupo"}</div>
          </div>
          <div>
            <label className="text-[10px] text-zinc-500 uppercase tracking-wider">Tipo</label>
            <div className="mt-1 rounded-lg border border-zinc-700 bg-zinc-900 px-2 py-1.5 text-xs text-zinc-300">{(editForm.player_type || (positionGroupFromOption(editForm.position, positionOptions) === "Arqueros" ? "arquero" : "jugador_campo")) === "arquero" ? "Arquero" : "Jugador de campo"}</div>
          </div>
          <div>
