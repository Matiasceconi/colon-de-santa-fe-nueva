...
export default function PlayerDatosPersonalesTab({ player, activeMembership, dayStatus }) {
  ...
  const residenceZone = player.residence_zone || (["AMBA", "INTERIOR", "EXTERIOR"].includes(player.current_residence) ? player.current_residence : null);
  const contractStatus = player.contract_status || (player.has_contract === true ? "Con contrato" : "Sin información");
  return (
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <div className="bg-zinc-900 rounded-xl px-4 divide-y divide-zinc-800/50">
            ...
            <InfoRow label="Zona de residencia" value={residenceZone} />
            <InfoRow label="Ciudad / localidad" value={player.city} />
            <InfoRow label="Provincia" value={player.province} />
            <InfoRow label="Tipo de pensión" value={player.housing_type || "Sin información"} />
            <InfoRow label="Situación contractual" value={contractStatus} />
            <InfoRow label="Celular" value={player.phone_number} />
          </div>
        </div>
        <div>
