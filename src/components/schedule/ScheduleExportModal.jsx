...
import { buildExportFileName } from "@/lib/exports/pdfExportKit";
...
export default function ScheduleExportModal({ open, onClose, activeSquad, eventsForDate, currentWeekStart, currentMonth }) {
  const { clubBrand: brand, can } = useWorkspace();
  ...
  const daysToExport = useMemo(() => {
    ...
    if (format === "weekly") {
      const start = rangeStart ? moment(rangeStart) : (currentWeekStart || moment().startOf("isoWeek"));
      const end = rangeEnd ? moment(rangeEnd) : start.clone().add(6, "days");
      if (!start.isValid() || !end.isValid() || end.isBefore(start, "day") || end.diff(start, "days") > 365) return [];
      const days = [];
      let d = start.clone();
  ...
  const configKey = JSON.stringify({ format, output, includeEmpty, singleDate, rangeStart, rangeEnd, month: currentMonth?.format("YYYY-MM"), squad: activeSquad?.id, brand, events: allEvents });
  const previewCurrent = Boolean(previewUrl && generatedKey === configKey);
  ...
  async function generate() {
    ...
    try {
      ...
      setError(err.message || "No se pudo generar el archivo. Volvé a intentarlo.");
    } finally {
      setGenerating(false);
    }
  }
  ...
  function download() {
    if (!previewCurrent || generating) return;
    try { assertExportAllowed(can, "/schedule"); } catch(e) { setError(e.message); return; }
    ...
    link.download = buildExportFileName(baseName, [brand.name, activeSquad?.name], output === "pdf" ? "pdf" : "png");
    link.click();
  }
  ...
  return (
    <ReportSurface onClose={onClose} className="fixed inset-0 z-[70] bg-black/70 flex items-start justify-center p-4 overflow-y-auto">
      <div className="w-full max-w-4xl my-4 bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          ...
          {previewUrl && !previewCurrent && <p role="status" className="text-sm text-amber-300">Cambiaste las opciones. Generá otra vista previa antes de descargar.</p>}
          {/* Botones */}
