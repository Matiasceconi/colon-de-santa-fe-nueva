...
function typeStyle(event, brand) {
  const text = norm(`${event.title || ""} ${event.event_type || ""} ${event.type || ""} ${event.location || ""}`);
  if (text.includes("partido")) return { icon: Trophy, label: "Partido", color: brand.colors.greenDeep, bg: "#E6F4EA" };
  ...
  return { icon: CalendarDays, label: event.event_type || event.type || "Actividad", color: brand.colors.green, bg: "#ECFDF3" };
}
...
function EventPrintCard({ event, brand }) {
  const style = typeStyle(event, brand);
          ...
          return (
            <div key={date} className="border rounded-xl overflow-hidden break-inside-avoid bg-white" style={{ borderColor: CLUB_BRAND.colors.line }}>
              ...
              <div className="p-2 space-y-2 min-h-[270px]" style={{ backgroundColor: events.length ? "#FFFFFF" : CLUB_BRAND.colors.panel }}>
                {events.length ? events.map((event) => <EventPrintCard key={event.id} event={event} brand={CLUB_BRAND} />) : <div className="h-full min-h-[230px] flex items-center justify-center text-center text-zinc-400"><div><p className="text-3xl">☾</p><p className="text-xs font-black mt-2">SIN EVENTOS</p></div></div>}
              </div>
            </div>
          );
        })}
