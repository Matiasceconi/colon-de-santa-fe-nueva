import { appendScheduleDetails } from "./schedulePdfDetails";
...
function drawHeader(doc, day, squadName, logo, brand) {
  ...
  setText(doc, contrastText(colors.primaryDeep)); doc.setFont("helvetica", "bolditalic"); doc.setFontSize(20);
  ...
  setText(doc, contrastText(colors.accent)); doc.setFont("helvetica", "bold"); doc.setFontSize(10.5);
  ...
  setText(doc, contrastText(colors.primaryDeep)); doc.setFont("helvetica", "bold"); doc.setFontSize(13.5);
...
function drawMatchSection(doc, ev, x, y, w, logo, brand) {
  ...
  setText(doc, contrastText(colors.primaryDeep)); doc.setFont("helvetica", "bold"); doc.setFontSize(9);
  ...
  setText(doc, contrastText(colors.primaryDeep)); doc.setFont("helvetica", "bold"); doc.setFontSize(13);
...
function drawTable(doc, events, brand) {
  ...
  for (let i = 0; i < rows; i++) {
    ...
    setText(doc, contrastText(colors.primaryDeep)); doc.setFont("helvetica", "bold"); doc.setFontSize(String(timeText(ev)).includes("\n") ? 7.8 : 10);
...
export async function buildDailySchedulePDF({ day, events, squadName, brand }) {
  ...
  const isMatch=e=>normalize(`${e.event_type} ${e.type} ${e.title}`).includes("partido");
  const pages=[];let group=[];
  for(const event of sorted){
    const next=[...group,event],matches=next.filter(isMatch).length,others=next.length-matches;
    if(group.length && 20+Math.max(others,1)*38+matches*64>186){pages.push(group);group=[];}
    group.push(event);
  }
  if(group.length||!pages.length)pages.push(group);
  for(let index=0;index<pages.length;index++){
    if(index)doc.addPage();
    setFill(doc,"#F7F7F5");doc.rect(0,0,210,297,"F");
    drawHeader(doc,day,squadName,logo,brand);
    drawTable(doc,pages[index],brand);
    const matches=pages[index].filter(isMatch),otherCount=pages[index].length-matches.length;
    for(let i=0;i<matches.length;i++){
      const rivalLogo=await loadSafeImage(matches[i].rival_logo_url);
      const matchY=66+14+38*Math.max(otherCount,1)+6+i*64;
      if(rivalLogo)addImageSafe(doc,rivalLogo,93,matchY+12,24,24);
    }
    drawFooter(doc,logo,brand);
  }
  await appendScheduleDetails(doc, { brand, days:[day], eventsForDate:()=>events });
  return doc;
}