...
export async function buildMonthSchedulePDF({ month, eventsForDate, squadName, season, brand, includeEmpty = true }) {
  ...
  doc.text(`Exportado el ${moment().format("DD/MM/YYYY")}${brand.showPerformancePitchBrand!==false?" · PerformancePitch":""}`, pw - 6, ph - 3.2, { align: "right" });

  await appendScheduleDetails(doc, { brand, days:days.filter(d=>d.isSame(month, "month")), eventsForDate });
  return doc;
}