...
function drawHeader(doc, days, squadName, season, brand) {
  ...
  setText(doc, contrastText(colors.primaryDeep));
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7.2);
  doc.text(doc.splitTextToSize(brand.name.toUpperCase(),88),192,12);
  doc.setFontSize(6.2);
  if(brand.showPerformancePitchBrand!==false)doc.text("PERFORMANCEPITCH", 241, 23);
  setFill(doc, colors.accent);
...
function drawFooter(doc, brand) {
  ...
  doc.text(`Exportado el ${exportedDate()}${brand.showPerformancePitchBrand!==false?" · PerformancePitch":""}`, pw - 6, ph - 3.2, { align: "right" });
}
...
export async function buildProfessionalWeekSchedulePDF({ days, eventsForDate, weekLabel, squadName, season, brand, includeEmpty = true }) {
  ...
  drawHeader(doc, chunk, squadName, season || "", brand);
  ...
  drawScheduleGrid(doc, chunk, eventsForDate, imageCache, brand);
