...
import { normalizeClubText, clubMatchesQuery } from "@/lib/rivalClubs";

export const EVENT_TYPES = ["Entrenamiento", "Partido", "Descanso", "Viaje", "Comida", "Video", "Gimnasio", "Reunión", "Evaluación", "Otro"];
export const TYPE_COLORS = { Partido: "red", Descanso: "cyan", Viaje: "purple", Comida: "orange", Video: "blue", Gimnasio: "purple", Entrenamiento: "green", Reunión: "yellow", Evaluación: "cyan", Otro: "blue" };
...
function normalizeType(value, title = "") {
  ...
  if (text.includes("reunión") || text.includes("reunion") || text.includes("charla") || text.includes("auditorio")) return "Reunión";
  if (text.includes("evaluación") || text.includes("evaluacion") || text.includes("control") || text.includes("test")) return "Evaluación";
...
function applyRestDayRule(events) {
  ...
  const result = [];
  Object.entries(byDate).forEach(([_date, dayEvents]) => {
