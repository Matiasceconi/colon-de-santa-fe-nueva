...
export default function ExerciseGPS({ session, exercise, sessionPlayers }) {
  ...
  useEffect(() => {
    ...
    base44.entities.ExerciseGPSData.filter({ exercise_id: exerciseId }, "player_name", 500).then(setRows);
  ...
  const blockNumbers = [...new Set(rows.map(row => Number(row.block_number)).filter(Number.isFinite))].sort((a, b) => a - b);
  const displayRows = selectedBlock === "all"
    ? (blockNumbers.length > 0 ? aggregateSessionGps(rows) : rows)
    : rows.filter(row => Number(row.block_number) === Number(selectedBlock));

  // Stats de la vista elegida: total del ejercicio o un bloque puntual.
  const avgDist = avg(displayRows, "total_distance");
  const avgMmin = avg(displayRows, "m_min");
  const avgD19  = avg(displayRows, "distance_19_8");
  const avgD25  = avg(displayRows, "distance_25");
  const avgSpr  = avg(displayRows, "sprints");
  const avgAcc  = avg(displayRows, "acc_3");
  const avgDec  = avg(displayRows, "dec_3");
  const avgLoad = avg(displayRows, "player_load");
  const maxSmax = maxVal(displayRows, "smax");
    ...
    <div className="mt-4 border-t border-zinc-700 pt-4 space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {rows.length > 0 && (
          <span className="text-[10px] text-zinc-500">
            {new Set(rows.map(row => row.player_id).filter(Boolean)).size} jugadores con GPS
            {blockNumbers.length > 1 ? ` · ${blockNumbers.length} bloques` : ""}
          </span>
        )}
      </div>
      ...
      {blockNumbers.length > 1 && (
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-[10px] text-zinc-500 uppercase tracking-wider font-medium">Carga externa</span>
          <button
          ...
          <span className="text-[10px] text-zinc-600">
            {selectedBlock === "all" ? "Suma de carga y velocidad máxima de todos los bloques." : "Datos del bloque seleccionado."}
          </span>
        </div>
      )}
