...
export default function FieldLibraryPanel() {
  ...
  useEffect(() => {
    Promise.all([
      base44.entities.FieldExerciseLibrary.filter({ squad_id: activeSquadId }, "-times_used", 500),
            ...
            <div key={ex.id} className="bg-zinc-800/50 border border-zinc-700 rounded-xl overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-3 text-[10px] text-zinc-500 flex-wrap">
                  {ex.format_label && <span className="text-zinc-300 font-medium">{ex.format_label}</span>}
                  {ex.length_m && ex.width_m && <span>{ex.length_m}×{ex.width_m} m</span>}
                  {ex.players_count && <span>{ex.players_count} participantes</span>}
                  {ex.eii && <span className="text-amber-400 font-semibold">EII {ex.eii} m²/jug</span>}
                    ...
                    {(activeTab[ex.id] || "info") === "info" && (
                      <div className="space-y-2">
                        ...
                        <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3">
                          <p className="text-[9px] uppercase tracking-wider font-semibold text-amber-300 mb-2">Configuración habitual · espacio e interacción</p>
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            {ex.format_label && <div><span className="text-zinc-500">Formato: </span><span className="text-zinc-300">{ex.format_label}</span></div>}
                            {ex.length_m && ex.width_m && <div><span className="text-zinc-500">Espacio: </span><span className="text-zinc-300">{ex.length_m}×{ex.width_m} m</span></div>}
                            {ex.total_area && <div><span className="text-zinc-500">Superficie: </span><span className="text-zinc-300">{ex.total_area} m²</span></div>}
                            {(ex.aspect_ratio || (ex.length_m && ex.width_m)) && <div><span className="text-zinc-500">Relación L/A: </span><span className="text-zinc-300">{ex.aspect_ratio || Number(ex.length_m / ex.width_m).toFixed(2)}:1</span></div>}
                            {ex.players_count && <div><span className="text-zinc-500">Participantes: </span><span className="text-zinc-300">{ex.players_count}</span></div>}
                            {(ex.eii_players_count || ex.players_count) && <div><span className="text-zinc-500">Jug. para EII: </span><span className="text-zinc-300">{ex.eii_players_count || ex.players_count}</span></div>}
                            {ex.eii && <div><span className="text-zinc-500">EII: </span><span className="font-bold text-amber-300">{ex.eii} m²/jug</span></div>}
                          </div>
                        </div>
                        <div className="grid grid-cols-2 gap-2 text-xs">
