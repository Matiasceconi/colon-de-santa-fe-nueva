...
import { syncToFieldLibrary } from "@/components/sessions/exerciseLibrarySync";
...
const EMPTY_FORM = {
  name: "", type: "Técnico", format_label: "", duration_min: "", blocks: "", work_time: "", rest_time: "",
  length_m: "", width_m: "", players_count: "", eii_players_count: "", objective: "", description: "",
...
function calcDerived(f) {
  ...
  const participants = parseFloat(f.players_count) || 0;
  const eiiPlayers = parseFloat(f.eii_players_count) || participants;
  const total_area = l && w ? parseFloat((l * w).toFixed(1)) : undefined;
  const eii = total_area && eiiPlayers ? parseFloat((total_area / eiiPlayers).toFixed(2)) : undefined;
  const aspect_ratio = l && w ? parseFloat((l / w).toFixed(2)) : undefined;
  ...
  return { total_area, eii, eiiPlayers: eiiPlayers || undefined, aspect_ratio, totalTime };
}

function deltaPercent(current, reference) {
...
export default function SessionExercises({ session, sessionPlayers }) {
  ...
  useEffect(() => {
    if (!session?.squad_id) { setLibraryMap({}); return; }
    base44.entities.FieldExerciseLibrary.filter({ squad_id: session.squad_id }, "-times_used", 500).then(rows => {
      setLibraryMap(Object.fromEntries(rows.map(row => [row.id, row])));
    }).catch(() => setLibraryMap({}));
  }, [session?.squad_id]);
  ...
  function setParticipants(value) {
    setForm(f => ({ ...f, players_count: value, eii_players_count: f.eii_players_count || value }));
  }
  ...
  function openEdit(ex) {
    setForm({
      name: ex.name || "", type: ex.type || "Técnico", format_label: ex.format_label || "",
      ...
      players_count: ex.players_count ?? "", eii_players_count: ex.eii_players_count ?? ex.players_count ?? "", objective: ex.objective || "",
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    setSaving(true);
    const { total_area, eii, aspect_ratio, totalTime } = calcDerived(form);
    const payload = {
      session_id: sessionId,
      name: form.name,
      type: form.type,
      format_label: form.format_label || undefined,
      ...
      eii_players_count: num(form.eii_players_count) ?? num(form.players_count),
      total_area,
      eii,
      aspect_ratio,
    ...
    if (editId) {
      ...
      const shouldUpdateLibrary = !!(linkedId && updateLibraryToo);
      if (shouldUpdateLibrary) {
        ...
        setLibraryMap(prev => ({ ...prev, [linkedId]: { ...(prev[linkedId] || {}), ...payload, id: linkedId } }));
      }
      ...
      if (libId) {
        if (updateLibraryToo) {
          await syncToFieldLibrary(form, sessionId, session?.squad_id, session?.squad_name, { updateExistingId: libId, incrementUsage: false });
          setLibraryMap(prev => ({ ...prev, [libId]: { ...(prev[libId] || {}), ...payload, id: libId } }));
        }
      } else {
  ...
  async function openLibrary() {
    const data = await base44.entities.FieldExerciseLibrary.filter({ squad_id: session?.squad_id }, "-times_used", 300);
    setLibraryExercises(data);
    setLibraryMap(prev => ({ ...prev, ...Object.fromEntries(data.map(row => [row.id, row])) }));
    setLibTypeFilter("");
    setShowLibrary(true);
  }
...
Formato: ${form.format_label || "no especificado"}
Participantes: ${form.players_count || "no especificado"}
Jugadores para EII: ${form.eii_players_count || form.players_count || "no especificado"}
Espacio: ${form.length_m || "?"}x${form.width_m || "?"} m
EII: ${calcDerived(form).eii || "no calculado"} m²/jug
...
Tipo: ${form.type}
Formato: ${form.format_label || "no especificado"}
Participantes: ${form.players_count || "?"}
Espacio: ${form.length_m || "?"}x${form.width_m || "?"} m
EII: ${calcDerived(form).eii || "no calculado"} m²/jug
Objetivo: ${form.objective || "no especificado"}
...
Formato de respuesta: "Objetivo táctico · Objetivo físico · Intensidad: Baja/Media/Alta". Sin comillas.`,
  ...
  const linkedLibraryForForm = form.library_exercise_id ? libraryMap[form.library_exercise_id] : null;
  const formEiiDelta = linkedLibraryForForm?.eii ? deltaPercent(derived.eii, linkedLibraryForForm.eii) : null;

  return (
    <div data-tour="session-exercises-root" className="space-y-4">
      ...
      {exercises.map((ex, i) => {
        ...
        const linkedLibrary = ex.library_exercise_id ? libraryMap[ex.library_exercise_id] : null;
        const exEiiDelta = linkedLibrary?.eii ? deltaPercent(ex.eii, linkedLibrary.eii) : null;
        return (
            ...
            <div className="flex items-center gap-3 p-3 cursor-pointer" onClick={() => toggleExpand(ex.id)}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 flex-wrap text-[10px] text-zinc-400">
                  ...
                  {ex.format_label && <span>Formato: <strong className="text-zinc-200">{ex.format_label}</strong></span>}
                  {ex.eii != null && (
                    <span className="px-1.5 py-0.5 rounded-full border border-amber-500/30 bg-amber-500/10 text-amber-300">
                      EII {ex.eii} m²/jug
                    </span>
                  )}
                  {exEiiDelta != null && Math.abs(exEiiDelta) >= 1 && (
                    <span className="text-zinc-500">{formatDelta(exEiiDelta)} vs habitual</span>
                  )}
                </div>
              </div>
              ...
              <div className="border-t border-zinc-700 p-4 space-y-3">
                ...
                <div className="rounded-xl border border-zinc-700 bg-zinc-900/40 p-3 space-y-2">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <p className="text-[10px] uppercase tracking-wider font-semibold text-amber-300">Espacio e interacción</p>
                    {ex.format_label && <span className="text-[10px] px-2 py-0.5 rounded-full border border-zinc-700 text-zinc-300">{ex.format_label}</span>}
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-6 gap-3 text-xs">
                    {ex.length_m && ex.width_m && <div><p className="text-zinc-500">Espacio</p><p className="text-white font-medium">{ex.length_m} × {ex.width_m} m</p></div>}
                    {ex.total_area && <div><p className="text-zinc-500">Superficie</p><p className="text-white font-medium">{ex.total_area} m²</p></div>}
                    {ex.aspect_ratio && <div><p className="text-zinc-500">Relación L/A</p><p className="text-white font-medium">{ex.aspect_ratio}:1</p></div>}
                    {ex.players_count && <div><p className="text-zinc-500">Participantes</p><p className="text-white font-medium">{ex.players_count}</p></div>}
                    {(ex.eii_players_count || ex.players_count) && <div><p className="text-zinc-500">Jug. para EII</p><p className="text-white font-medium">{ex.eii_players_count || ex.players_count}</p></div>}
                    {ex.eii && <div><p className="text-zinc-500">EII</p><p className="text-amber-300 font-bold">{ex.eii} m²/jug</p></div>}
                  </div>
                  {linkedLibrary?.eii && exEiiDelta != null && Math.abs(exEiiDelta) >= 1 && (
                    <p className="text-[10px] text-zinc-500">Configuración habitual de biblioteca: {linkedLibrary.eii} m²/jug · hoy {formatDelta(exEiiDelta)}.</p>
                  )}
                </div>
      ...
      {showForm ? (
        <form onSubmit={handleSubmit} className="bg-zinc-800/50 border border-zinc-700 rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            />
            <div>
              <label className="text-[10px] text-zinc-400 mb-1 block">Formato</label>
              <input value={form.format_label} onChange={e => setF("format_label", e.target.value)}
                placeholder="Ej: 5v5 + 2 ARQ / 6v6 + 3C"
                className="w-full bg-zinc-700 border border-zinc-600 rounded-lg px-3 py-2 text-xs text-white focus:outline-none" />
            </div>

            <div className="col-span-2 rounded-xl border border-zinc-700 bg-zinc-900/35 p-3">
              <p className="text-[10px] uppercase tracking-wider font-semibold text-zinc-400 mb-2">Organización de la tarea</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-zinc-400 mb-1 block">Participantes totales</label>
                  <input type="number" min={1} value={form.players_count} onChange={e => setParticipants(e.target.value)}
                    placeholder="Ej: 12"
                    className="w-full bg-zinc-700 border border-zinc-600 rounded-lg px-3 py-2 text-xs text-white focus:outline-none" />
                  <p className="mt-1 text-[9px] text-zinc-600">Cantidad total que realiza la tarea.</p>
                </div>
                <div>
                  <label className="text-[10px] text-zinc-400 mb-1 block">Jugadores considerados para EII</label>
                  <input type="number" min={1} value={form.eii_players_count} onChange={e => setF("eii_players_count", e.target.value)}
                    placeholder={form.players_count ? `Por defecto ${form.players_count}` : "Ej: 10"}
                    className="w-full bg-zinc-700 border border-zinc-600 rounded-lg px-3 py-2 text-xs text-white focus:outline-none" />
                  <p className="mt-1 text-[9px] text-zinc-600">Editable para excluir ARQ, comodines u otros participantes según el criterio del club.</p>
                </div>
              </div>
            </div>
            ...
            {/* Espacio e interacción individual */}
            <div className="col-span-2 rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 space-y-3">
              <div>
                <p className="text-[10px] uppercase tracking-wider font-semibold text-amber-300">Espacio e interacción individual</p>
                <p className="mt-0.5 text-[9px] text-zinc-500">El EII se calcula con la superficie de la tarea dividida por los jugadores que el staff decide considerar en el denominador.</p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-zinc-400 mb-1 block">Largo (m)</label>
                  <input type="number" min={0} step="0.1" value={form.length_m} onChange={e => setF("length_m", e.target.value)}
                    className="w-full bg-zinc-700 border border-zinc-600 rounded-lg px-3 py-2 text-xs text-white focus:outline-none" />
                </div>
                <div>
                  <label className="text-[10px] text-zinc-400 mb-1 block">Ancho (m)</label>
                  <input type="number" min={0} step="0.1" value={form.width_m} onChange={e => setF("width_m", e.target.value)}
                    className="w-full bg-zinc-700 border border-zinc-600 rounded-lg px-3 py-2 text-xs text-white focus:outline-none" />
                </div>
              </div>

              {(derived.total_area || derived.eii || derived.aspect_ratio) && (
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                  <DerivedMetric label="Superficie" value={derived.total_area != null ? `${derived.total_area} m²` : "—"} />
                  <DerivedMetric label="Jug. para EII" value={derived.eiiPlayers || "—"} />
                  <DerivedMetric label="EII" value={derived.eii != null ? `${derived.eii} m²/jug` : "—"} accent />
                  <DerivedMetric label="Relación L/A" value={derived.aspect_ratio != null ? `${derived.aspect_ratio}:1` : "—"} />
                </div>
              )}

              {linkedLibraryForForm?.eii && derived.eii && (
                <div className="rounded-lg border border-zinc-700 bg-zinc-900/50 px-3 py-2 text-[10px] text-zinc-400">
                  Configuración habitual de biblioteca: <strong className="text-zinc-200">{linkedLibraryForForm.eii} m²/jug</strong>
                  {formEiiDelta != null && Math.abs(formEiiDelta) >= 1 && <span> · hoy <strong className="text-amber-300">{formatDelta(formEiiDelta)}</strong></span>}
                  <span className="block mt-1 text-zinc-600">Modificar este ejercicio no cambia la biblioteca salvo que actives “Actualizar también en biblioteca”.</span>
                </div>
              )}
            </div>
        ...
        <div data-tour="session-exercises-actions" className="flex items-center gap-2 flex-wrap">
              ...
              {libraryExercises
                ...
                <div key={ex.id}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-zinc-600">{ex.format_label ? `${ex.format_label} · ` : ""}✓ {ex.times_used || 1} usos</p>
                  </div>
                  <div className="text-[10px] text-zinc-500 shrink-0 text-right">
                    {ex.length_m && ex.width_m && <p>{ex.length_m}×{ex.width_m} m</p>}
                    {ex.players_count && <p>{ex.players_count} participantes</p>}
                    {ex.eii && <p className="text-amber-400 font-semibold">EII {ex.eii} m²/jug</p>}
                  </div>
                </div>
              ))}
...
function DerivedMetric({ label, value, accent = false }) {
  return (
    <div className="rounded-lg border border-zinc-700 bg-zinc-900/50 px-2.5 py-2">
      <p className="text-[9px] uppercase tracking-wide text-zinc-600">{label}</p>
      <p className={`mt-0.5 text-xs font-bold ${accent ? "text-amber-300" : "text-zinc-100"}`}>{value}</p>
    </div>
  );
}