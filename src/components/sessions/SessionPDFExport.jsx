...
import { X, Printer, Calendar, Clock, Users, MapPin, Dumbbell, Video } from "lucide-react";
...
function Section({ title, children }) {
  return <section className="break-inside-avoid rounded-2xl border border-zinc-200 bg-white p-4"><h2 className="text-sm font-black uppercase tracking-wide mb-3" style={{ color: "#0F172A" }}>{title}</h2>{children}</section>;
}
...
export default function SessionPDFExport({ session, sessionPlayers, onClose }) {
  ...
  const { clubBrand: CLUB_BRAND } = useWorkspace();
        ...
        {loading ? <div className="text-center py-12 text-zinc-500">Cargando vista previa...</div> : (
          <div className="space-y-5">
            ...
            <Section title="Ejercicios de campo">
              {data.exercises.length ? <div className="grid md:grid-cols-2 gap-3">{data.exercises.map((ex, idx) => <div key={ex.id} className="rounded-xl border border-zinc-200 p-3"><p className="font-black text-sm">{idx + 1}. {ex.name}</p><p className="text-xs text-zinc-500 mt-1">{[ex.format_label, ex.duration_min && `${ex.duration_min} min`, ex.blocks && `${ex.blocks} bloques`, ex.players_count && `${ex.players_count} participantes`].filter(Boolean).join(" · ")}</p>{(ex.length_m || ex.eii) && <div className="mt-2 rounded-lg bg-amber-50 border border-amber-100 p-2 text-xs text-zinc-700">{ex.length_m && ex.width_m && <span>{ex.length_m}×{ex.width_m} m · </span>}{ex.total_area && <span>{ex.total_area} m² · </span>}{ex.eii && <strong>EII {ex.eii} m²/jug</strong>}{(ex.eii_players_count || ex.players_count) && <span> · n EII {ex.eii_players_count || ex.players_count}</span>}</div>}{ex.description && <p className="text-xs text-zinc-600 mt-2">{ex.description}</p>}</div>)}</div> : <p className="text-sm text-zinc-500">Sin ejercicios cargados.</p>}
            </Section>
