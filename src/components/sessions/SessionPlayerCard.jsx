...
import { STATUS_LABELS, STATUS_COLORS, SESSION_STATUS_OPTIONS, ATTENDANCE_LABELS } from "./sessionPlayerUtils";
...
export default function SessionPlayerCard({ sp, photoUrl, hasGps, onAction, onSaveDetails, onStatusChange }) {
    ...
    <div className="bg-zinc-800/50 border border-zinc-700 rounded-xl p-3 space-y-2.5">
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-[10px]">
        ...
        {sp.attendance === "presente" && (
          hasGps ? (
            <span className="px-2 py-0.5 rounded-full border font-medium bg-cyan-500/15 text-cyan-300 border-cyan-500/30">
              GPS
            </span>
          ) : (
            <span className="px-2 py-0.5 rounded-full border font-medium bg-zinc-700/40 text-zinc-400 border-zinc-600">
              Sin GPS
            </span>
          )
        )}
      ...
      <div className="pt-1 border-t border-zinc-700/60 space-y-2">
        <div className="flex items-center justify-between gap-2">
          <span className="text-[9px] uppercase tracking-wide text-zinc-600">Trabajo del día</span>
          <span className="text-[10px] font-semibold text-zinc-300">{ATTENDANCE_LABELS[sp.attendance] || sp.attendance || "—"}</span>
        </div>
        <div className="flex items-center gap-1 flex-wrap">
          {ACTIONS.map(({ key, label, icon: Icon, activeClass }) => (
            <button key={key} type="button" onClick={() => onAction(sp, key)} title={label}
          ...
          <button type="button" onClick={() => setExpanded(e => !e)} title="Estado previo, minutos, RPE y notas"
      ...
      {expanded && (
        <div className="space-y-2 pt-1">
          <div>
            <label className="text-[9px] uppercase tracking-wide text-zinc-600 block mb-1">Estado al inicio de la sesión</label>
            <select value={sp.status_at_session || "disponible"} onChange={e => onStatusChange?.(sp, e.target.value)}
              className="w-full bg-zinc-900 border border-zinc-700 rounded px-2 py-1.5 text-[11px] text-white focus:outline-none">
              {SESSION_STATUS_OPTIONS.map(status => <option key={status} value={status}>{STATUS_LABELS[status] || status}</option>)}
            </select>
          </div>
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={1} placeholder="Observación..."
            className="w-full bg-zinc-900 border border-zinc-700 rounded px-1.5 py-1 text-[11px] text-white focus:outline-none resize-none" />
        </div>
      )}
    </div>
  );
