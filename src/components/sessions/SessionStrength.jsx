...
import { canonicalExerciseName, findSimilarStrengthExercise } from "@/components/sessions/exerciseLibrarySync";
...
export default function SessionStrength({ session, onSessionUpdate }) {
  ...
  async function loadData() {
    ...
    // Normalización no destructiva: `series` es la prescripción canónica.
    // Si una fila histórica tiene series/reps planos válidos, se materializa la misma prescripción en `series`.
    const normalizedRows = [];
    for (const row of nextRows) {
      if ((!row.series || !row.series.length) && parseInt(row.sets, 10) > 0) {
        const migrated = migrateSeries(row);
        if (migrated.length) {
          try {
            normalizedRows.push(await base44.entities.StrengthStation.update(row.id, { series: migrated, sets: String(migrated.length) }));
            continue;
          } catch { /* conservar fila original si falla la normalización */ }
        }
      }
  ...
  const strengthSummary = useMemo(() => {
    const named = stations.filter((row) => String(row.exercise_name || "").trim());
    const totalSeries = named.reduce((sum, row) => sum + migrateSeries(row).length, 0);
    const withoutPrescription = named.filter((row) => migrateSeries(row).length === 0).length;
    const groups = new Map();
    named.forEach((row) => {
      const canonical = canonicalExerciseName(row.exercise_name);
      if (!canonical) return;
      const key = `${row.work_block_id || uidName(row.strength_group)}::${canonical}`;
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key).push(row);
    });
    const duplicates = [...groups.values()].filter((rows) => rows.length > 1);
    return {
      blocks: visibleBlocks.filter((block) => !block.hidden).length,
      exercises: named.length,
      totalSeries,
      withoutPrescription,
      duplicates,
    };
  }, [stations, visibleBlocks]);
  ...
  async function onBlurField(station) {
    ...
    const payload = {
      ...
      sets: station.series?.length ? String(station.series.length) : (station.sets || undefined),
  ...
  async function onPickLibrary(id, ex) {
    ...
    // Elegir desde biblioteca vincula la identidad y registra el uso.
    // Nunca reemplaza la prescripción `series` que pertenece a esta sesión.
    await syncToLibrary({ ...current, ...updated }, session.id, session.squad_id, session.squad_name, { updateExistingId: ex.id, session, updateMetadata: false });
  }
  ...
  return (
    <div data-tour="session-strength-root" className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div>
          <h3 className="text-white font-semibold text-sm">Fuerza · planificación de la sesión</h3>
          <p className="text-xs text-zinc-500">Biblioteca = identidad del ejercicio · Sesión = prescripción real · Plantillas = rutinas reutilizables.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-2">
        {[
          { label: "Cuadros", value: strengthSummary.blocks, tone: "text-blue-300" },
          { label: "Ejercicios", value: strengthSummary.exercises, tone: "text-white" },
          { label: "Series", value: strengthSummary.totalSeries, tone: "text-emerald-300" },
          { label: "Sin prescripción", value: strengthSummary.withoutPrescription, tone: strengthSummary.withoutPrescription ? "text-amber-300" : "text-zinc-400" },
          { label: "Duplicados a revisar", value: strengthSummary.duplicates.length, tone: strengthSummary.duplicates.length ? "text-orange-300" : "text-zinc-400" },
        ].map((item) => (
          <div key={item.label} className="rounded-xl border border-zinc-800 bg-zinc-900/50 px-3 py-2.5">
      ...
      {strengthSummary.duplicates.length > 0 && mode === "edit" && (
        <div className="rounded-xl border border-orange-500/25 bg-orange-500/5 p-3">
          <p className="text-xs font-semibold text-orange-200">Revisión de integridad</p>
          <p className="mt-1 text-[10px] text-zinc-500">Hay ejercicios repetidos dentro del mismo cuadro. No los eliminamos automáticamente porque pueden representar prescripciones distintas.</p>
          <div className="mt-2 flex flex-wrap gap-1.5">
            {strengthSummary.duplicates.slice(0, 8).map((rows) => (
              <span key={`${rows[0].work_block_id}-${canonicalExerciseName(rows[0].exercise_name)}`} className="rounded-lg border border-orange-500/20 bg-zinc-900/60 px-2 py-1 text-[10px] text-orange-100">
                {rows[0].exercise_name} × {rows.length}
              </span>
            ))}
          </div>
        </div>
      ...
      {mode === "edit" && (
        <div data-tour="session-strength-actions" className="flex items-center gap-2 flex-wrap">
