...
import { Star, Search, ChevronDown, ChevronUp, Archive, RotateCcw, GitMerge, Play } from "lucide-react";
...
export default function StrengthLibraryPanel() {
  ...
  useEffect(() => {
    base44.entities.StrengthExerciseLibrary.list("-times_used", 500).then(data => {
      ...
      // Normalización en memoria para visualización. La persistencia del campo
      // normalized_name se realiza al crear/editar ejercicios, no en cada carga
      // de la biblioteca (evita ráfagas de escritura que disparan el rate limit).
      const normalized = visible.map(exercise => {
        if (!exercise.normalized_name && exercise.name) {
          return { ...exercise, normalized_name: canonicalExerciseName(exercise.name) };
        }
        return exercise;
      });
  ...
  async function handleArchive(ex) {
    if (!window.confirm(`¿Archivar "${ex.name}"? Las sesiones históricas conservarán el vínculo.`)) return;
    const updated = await base44.entities.StrengthExerciseLibrary.update(ex.id, { active: false });
    setExercises(prev => prev.map(e => e.id === ex.id ? { ...e, ...updated } : e));
    toast({ title: "✓ Ejercicio archivado" });
  }

  async function handleRestore(ex) {
    const updated = await base44.entities.StrengthExerciseLibrary.update(ex.id, { active: true });
    setExercises(prev => prev.map(e => e.id === ex.id ? { ...e, ...updated } : e));
  ...
  async function mergeExercises(source, target) {
    ...
    const templates = await base44.entities.StrengthBlockTemplate.list("-created_at", 300).catch(() => []);
    for (const template of templates) {
      if (!(template.exercises || []).some(item => item.library_exercise_id === source.id)) continue;
      await base44.entities.StrengthBlockTemplate.update(template.id, {
        exercises: (template.exercises || []).map(item => item.library_exercise_id === source.id ? { ...item, library_exercise_id: target.id } : item),
      });
    }
  ...
  const filtered = exercises
    .filter(e => {
      if (!showArchived && e.active === false) return false;
  ...
  const allMethods = [...new Set(activeExercises.map(e => e.method).filter(Boolean))];
  ...
  const missingNormalized = activeExercises.filter(ex => !ex.normalized_name).length;
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Ejercicios activos", value: activeExercises.length, color: "text-orange-400" },
          { label: "Duplicados a revisar", value: duplicateGroups.length, color: duplicateGroups.length ? "text-amber-400" : "text-zinc-400" },
          { label: "Sin normalizar", value: missingNormalized, color: missingNormalized ? "text-blue-300" : "text-zinc-400" },
        ].map(s => (
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <label className="flex items-center gap-2 rounded-lg border border-zinc-700 bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-400">
          <input type="checkbox" checked={showArchived} onChange={e => setShowArchived(e.target.checked)} /> Archivados
        </label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {filtered.map(ex => {
          ...
          const possibleDuplicates = exercises.filter(other => other.active !== false && other.id !== ex.id && canonicalExerciseName(other.name) === canonicalExerciseName(ex.name));
          return (
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex gap-1 shrink-0">
                    ...
                    {ex.active === false ? (
                      <button onClick={() => handleRestore(ex)} className="p-1 text-zinc-500 hover:text-emerald-400 transition-colors" title="Restaurar">
                        <RotateCcw size={13} />
                      </button>
                    ) : (
                      <button onClick={() => handleArchive(ex)} className="p-1 text-zinc-500 hover:text-amber-400 transition-colors" title="Archivar sin romper sesiones históricas">
                        <Archive size={13} />
                      </button>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-3 text-[10px] text-zinc-500 flex-wrap">
                  ...
                  {ex.vector_pattern && <span>{ex.vector_pattern}</span>}
                  {ex.active === false && <span className="rounded border border-zinc-700 px-1.5 py-0.5 text-zinc-500">Archivado</span>}
                ...
                {isExp && (
                  <div className="mt-3 pt-3 border-t border-zinc-700 space-y-2">
                    ...
                    <div className="rounded-lg border border-zinc-700 bg-zinc-900/50 p-2">
                      <p className="text-[10px] font-semibold text-zinc-400">Identidad del ejercicio</p>
                      <div className="mt-1 grid grid-cols-2 gap-2 text-xs">
                        {ex.category && <div><span className="text-zinc-500">Categoría: </span><span className="text-zinc-300">{ex.category}</span></div>}
                        {ex.exercise_type && <div><span className="text-zinc-500">Tipo: </span><span className="text-zinc-300">{ex.exercise_type}</span></div>}
                        {ex.method && <div><span className="text-zinc-500">Método: </span><span className="text-zinc-300">{ex.method}</span></div>}
                        ...
                        {ex.vector_pattern && <div><span className="text-zinc-500">Patrón: </span><span className="text-zinc-300">{ex.vector_pattern}</span></div>}
                      </div>
                      <p className="mt-2 text-[9px] text-zinc-600">La carga, series, repeticiones y pausas se prescriben en cada sesión o plantilla; no se sobrescriben desde aquí.</p>
                    </div>
