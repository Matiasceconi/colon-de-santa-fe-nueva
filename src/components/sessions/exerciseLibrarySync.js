...
function calcArea(form) {
  ...
  const participants = num(form.players_count);
  const eiiPlayers = num(form.eii_players_count) || participants;
  ...
  const aspect_ratio = length && width ? parseFloat((length / width).toFixed(2)) : undefined;
  return { total_area, eii, aspect_ratio, eiiPlayers };
}

export function buildFieldLibraryPayload(form, sessionId, squadId, squadName) {
  const { total_area, eii, aspect_ratio, eiiPlayers } = calcArea(form);
  return {
    name: form.name,
...
export function canonicalExerciseName(value) {
  ...
  if (text === "bulgara" || text === "sentadilla bulgara" || text === "split squat") return "sentadilla bulgara";
  text = text.replace(/\bsplit squat\b/g, "sentadilla bulgara");
  text = text.replace(/\bmanc\b/g, "mancuernas").replace(/\bmancuerna\b/g, "mancuernas");
  return text;
}
...
export async function findSimilarStrengthExercise(station, squadId) {
  ...
  const visible = all.filter(item => item.active !== false && (item.global === true || item.squad_id === squadId));
...
function buildStrengthLibraryUpdate(existing, payload, sessionId, options) {
  ...
  const overwriteMetadata = options.updateMetadata === true;
  const keepOrFill = (key) => overwriteMetadata ? (payload[key] ?? existing[key]) : (existing[key] ?? payload[key]);
  return {
    category: keepOrFill("category"),
    method: keepOrFill("method"),
    exercise_type: keepOrFill("exercise_type"),
    description: keepOrFill("description"),
    image_url: keepOrFill("image_url"),
    video_url: keepOrFill("video_url"),
    thumbnail_url: keepOrFill("thumbnail_url"),
    objective: keepOrFill("objective"),
    muscle_group: keepOrFill("muscle_group"),
    vector_pattern: keepOrFill("vector_pattern"),
    notes: keepOrFill("notes"),
    tags: overwriteMetadata && payload.tags?.length ? payload.tags : (existing.tags?.length ? existing.tags : payload.tags),
    aliases,
...
export async function syncToStrengthLibrary(station, sessionId, squadId, squadName, options = {}) {
  ...
  const match = linkedId
    ? all.find(item => item.id === linkedId)
    : all.find(item => item.active !== false && sameStrengthTemplate(item, station, squadId));
  if (match) {
