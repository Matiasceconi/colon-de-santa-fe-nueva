...
import { DEFAULT_TEMPLATE_NAMES, MAIN_FIELDS, saveTemplate } from "@/components/sessions/gps/gpsColumnsConfig";
...
export default function GpsImportPreview({ preview, selectedFields, setSelectedFields, templates, onTemplatesChange, importing, onCancel, onConfirm }) {
  ...
  const totalBlocks = preview.taskMode
    ? preview.tasks.reduce((sum, task) => sum + task.periods, 0)
    : 0;
      ...
      {preview.taskMode && (
        <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
              <p className="text-xs font-semibold text-blue-200">Archivo GPS por tareas detectado</p>
              <p className="text-[10px] text-blue-300/70 mt-1">
                Se crearán {preview.tasks.length} ejercicios con {totalBlocks} bloques. El total de cada jugador seguirá en la carga global.
              </p>
            </div>
            <span className="text-[10px] font-semibold text-blue-200 bg-blue-500/15 border border-blue-500/25 rounded-full px-2.5 py-1">
              {preview.parsedRows.length} registros
            </span>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 mt-3">
            {preview.tasks.map(task => (
              <div key={task.normalizedTask} className="bg-zinc-950/50 border border-blue-500/15 rounded-lg px-3 py-2 flex items-center justify-between gap-2">
                ...
                <span className="text-[10px] text-blue-300 whitespace-nowrap">
                  {task.periods} {task.periods === 1 ? "bloque" : "bloques"}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="flex gap-2 justify-end">
        ...
        <button onClick={onConfirm} disabled={importing}
          ...
          {importing
            ? "Importando..."
            : preview.taskMode
              ? `Crear ${preview.tasks.length} ejercicios · ${totalBlocks} bloques`
              : `Confirmar e importar (${parsedRows.length} jugadores)`}
        </button>
      </div>
    </div>
  );
