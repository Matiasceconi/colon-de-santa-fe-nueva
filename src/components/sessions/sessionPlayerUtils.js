...
export const SESSION_STATUS_OPTIONS = ["disponible", "diferenciado", "molestia", "lesionado", "reintegro", "suspendido", "ausente"];
...
export const ATTENDANCE_OPTIONS = ["presente", "diferenciado", "kinesiologia", "ausente", "no_entrena"];

export function normalizeSessionStatus(value) {
  const raw = String(value || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim();
  if (!raw || raw === "disponible" || raw.includes("alta med")) return "disponible";
  if (raw.includes("lesion")) return "lesionado";
  if (raw.includes("molest")) return "molestia";
  if (raw.includes("difer")) return "diferenciado";
  if (raw.includes("recuper") || raw.includes("reinteg")) return "reintegro";
  if (raw.includes("suspend")) return "suspendido";
  if (raw.includes("descanso") || raw.includes("permiso") || raw.includes("ausente")) return "ausente";
  return "disponible";
}

export function defaultAttendanceForStatus(status) {
  if (status === "lesionado") return "kinesiologia";
  if (["diferenciado", "molestia", "reintegro"].includes(status)) return "diferenciado";
  if (status === "ausente") return "ausente";
  if (status === "suspendido") return "no_entrena";
  return "presente";
}
