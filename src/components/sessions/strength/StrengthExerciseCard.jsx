            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-1.5 flex-wrap mt-0.5">
                  {station.exercise_type && <span className="rounded bg-zinc-800 px-1.5 py-0.5 text-[9px] text-zinc-400">{station.exercise_type}</span>}
                  {station.method && <span className="rounded bg-blue-500/10 px-1.5 py-0.5 text-[9px] text-blue-300">{station.method}</span>}
                  <span className="text-xs text-zinc-500 truncate">{series.length} series · {summarizeSeries(series)} · {summarizeLoads(series)}</span>
                </div>
              </div>
          ...
          {expanded && (
            <div className="border-t border-zinc-800 p-3 space-y-3">
              ...
              <div className="rounded-lg border border-zinc-800 bg-zinc-950/35 p-3 space-y-2">
                <div>
                  <p className="text-[10px] uppercase tracking-wider font-semibold text-zinc-400">Clasificación del ejercicio</p>
                  <p className="text-[9px] text-zinc-600">Describe qué es el ejercicio; no define la carga de esta sesión.</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <label className="block">
                    <span className="text-[10px] text-zinc-500">Método</span>
                    <input list="strength-method-options" value={station.method || ""} onChange={(e) => updateField("method", e.target.value)} onBlur={() => onBlur(station)} placeholder="Dinámicos, Isométricos..."
                      className="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none" />
                  </label>
                  <label className="block">
                    <span className="text-[10px] text-zinc-500">Tipo de trabajo</span>
                    <input list="strength-type-options" value={station.exercise_type || ""} onChange={(e) => updateField("exercise_type", e.target.value)} onBlur={() => onBlur(station)} placeholder="Potencia, Fuerza máxima..."
                      className="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none" />
                  </label>
                  <label className="block">
                    <span className="text-[10px] text-zinc-500">Categoría</span>
                    <input value={station.category || ""} onChange={(e) => updateField("category", e.target.value)} onBlur={() => onBlur(station)} placeholder="Tren inferior, Core..."
                      className="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none" />
                  </label>
                  <label className="block">
                    <span className="text-[10px] text-zinc-500">Patrón / vector</span>
                    <input value={station.vector_pattern || ""} onChange={(e) => updateField("vector_pattern", e.target.value)} onBlur={() => onBlur(station)} placeholder="Vertical, Horizontal, Rotacional..."
                      className="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none" />
                  </label>
                </div>
              </div>
              ...
              <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-3 space-y-3">
                <div>
                  <p className="text-[10px] uppercase tracking-wider font-semibold text-emerald-300">Prescripción de esta sesión</p>
                  <p className="text-[9px] text-zinc-600">Esta carga pertenece a la sesión. No modifica la biblioteca del ejercicio.</p>
                </div>
                <div className="flex flex-wrap items-end gap-3">
                  ...
                  <label className="block">
                    <span className="text-[10px] text-zinc-500">RIR objetivo</span>
                    <input value={station.rir || ""} onChange={(e) => updateField("rir", e.target.value)} onBlur={() => onBlur(station)} placeholder="Ej: 2"
                      className="mt-1 w-20 bg-zinc-800 border border-zinc-700 rounded-lg px-2 py-1.5 text-xs text-white focus:outline-none" />
                  </label>
                </div>
                <div>
