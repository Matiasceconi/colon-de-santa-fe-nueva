...
export default function StrengthSeriesTable({ series = [], onChange }) {
  ...
  const [genLoad, setGenLoad] = useState({ load_type: "none", load_value: "" });
  ...
  function generate() {
    ...
    const template = {
      reps: genReps || "",
      time: genTime || "",
      load_type: genLoad.load_type || "none",
      load_value: genLoad.load_value || "",
    };
    onChange(generateSeries(count, template));
    setGenCount("");
    setGenReps("");
    setGenTime("");
    setGenLoad({ load_type: "none", load_value: "" });
  }
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap items-end gap-2 rounded-lg border border-zinc-700 bg-zinc-950/40 p-2.5">
        ...
        <div>
          <label className="block text-[10px] text-zinc-500 mb-0.5">Carga para todas</label>
          <StrengthLoadSelector value={genLoad} onChange={setGenLoad} compact />
        </div>
        <button
          type="button"
          onClick={generate}
