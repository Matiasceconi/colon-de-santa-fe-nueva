...
import { ArrowLeft, Plus, ChevronDown, ChevronUp, Play, Users, FileText, FileDown, Pencil, X, ImagePlus } from "lucide-react";
...
async function exportSessionPDF(session, exercises, availablePlayers, clubBrand) {
  ...
  function addHeader(isFirst = false) {
    ...
    doc.text(`${clubName} — Cuerpo Técnico`, M, 20);
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.text("PerformancePitch", M, 290);
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button
            onClick={async () => {
              ...
              await exportSessionPDF(session, exercises, availablePlayers, clubBrand);
