...
import { buildTacticalExport } from "@/components/tactical/lib/tacticalReport";
...
export default function TacticalExportModal({ open, onClose, stageRef, boards, currentBoardId, brand, project }) {
  ...
  async function handleExport() {
    ...
    setError("");
    try {
      assertExportAllowed(can, "/tactical");
      const stage = stageRef.current?.getStage();
      if (!stage) throw new Error("No hay lienzo para exportar");
      const result = await buildTacticalExport({stage,boards,currentBoardId,scope,format,size,quality,brand,project});
      if (format === "clipboard") {
        if (!navigator.clipboard || !window.ClipboardItem) throw new Error("El navegador no permite copiar imágenes.");
  ...
  return (
    <ReportSurface onClose={onClose} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-zinc-900 border border-zinc-700 rounded-xl w-full max-w-md shadow-2xl" onClick={(e) => e.stopPropagation()}>
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {[{ id: "png", label: "PNG" }, { id: "jpg", label: "JPG" }, { id: "pdf", label: "PDF" }, { id: "clipboard", label: "Portapapeles" }].map((f) => (
                <button key={f.id} onClick={() => { setFormat(f.id); setScope("current"); if(f.id==="pdf") setSize("a4_horizontal"); }} className={`px-3 py-2 rounded-lg text-xs font-medium border transition-colors ${format === f.id ? "bg-white text-zinc-900 border-white" : "bg-zinc-800 text-zinc-300 border-zinc-700 hover:border-zinc-500"}`}>{f.label}</button>
              ))}
            </div>
          </div>
          {format !== "clipboard" && (
            <>
              <div>
                ...
                <select value={size} onChange={(e) => setSize(e.target.value)} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none">
                  {SIZES.filter(s => format !== "pdf" || s.id.startsWith("a4_")).map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
                </select>
              </div>
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button disabled={format !== "pdf" || boards.length < 2} onClick={() => setScope("all")} className={`px-3 py-2 rounded-lg text-xs font-medium border transition-colors ${scope === "all" ? "bg-white text-zinc-900 border-white" : "bg-zinc-800 text-zinc-300 border-zinc-700"}`}>Todas en un PDF</button>
                </div>
              </div>
              <div>
        ...
        <div className="flex justify-end gap-2 p-5 border-t border-zinc-800">
          ...
          <button onClick={handleExport} disabled={exporting || !can("export", "/tactical")} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm bg-white text-zinc-900 font-semibold hover:bg-zinc-200 disabled:opacity-40">
