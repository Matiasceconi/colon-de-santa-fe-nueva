...
const LEGACY_MODULE_PATHS = {
  ...
  performance: ["/performance/dashboard", "/performance/external-load", "/performance/internal-load", "/performance/minutes", "/performance/microcycle-history"],
...
const PATH_AREA_MAP = {
  "/performance/dashboard": "rendimiento_fisico",
...
export function WorkspaceProvider({ children }) {
  ...
  useEffect(() => {
    if (isAuthenticated && user && !demoActive) loadInstitutionProfile();
  }, [isAuthenticated, user, loadInstitutionProfile, demoActive]);
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    // Modo demo: el visitante no tiene StaffAccess, roles ni planteles reales.
    // Se provee un contexto ficticio (Performance FC · Primera) sin leer datos del club,
    // para no mezclar la demo con información real ni requerir permisos.
    if (demoActive) {
      setSquads([]);
      setActiveSquadState({ id: "demo-primera", name: "Primera", season: "2026" });
      setUserAccess(null);
      setRoleNames([]);
    ...
    try {
      // 1. Fetch active squads and institution-level module switches.
      // If InstitutionModule is unavailable/empty, modules remain enabled by default
      // so a configuration problem can never hide the existing product unexpectedly.
      const [allSquads, institutionModuleRows] = await Promise.all([
        base44.entities.Squad.filter({ active: true }, "name", 100),
        base44.entities.InstitutionModule.list("order", 200).catch(() => []),
      ]);
      setSquads(allSquads);
      setInstitutionModules(institutionModuleRows);
  ...
  function isInstitutionModuleEnabled(path) {
    if (!path) return true;
    const page = PAGES.find(p => p.path === path);
    if (!page?.module_id) return true;
    const config = institutionModules.find(row => row.module_id === page.module_id);
    return config ? config.enabled !== false : true;
  }

  function can(action, path = null) {
    if (path && !isInstitutionModuleEnabled(path)) return false;
  ...
  function canSeePath(path) {
    if (!isInstitutionModuleEnabled(path)) return false;
  ...
  if (!demoActive && !effectiveAdminAccess && myAreas.length === 0 && allowedPages.length === 0) {
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-6">
        <div className="w-full max-w-sm text-center space-y-4">
          <div className="w-12 h-12 rounded-full bg-yellow-500/10 border border-yellow-500/30 flex items-center justify-center mx-auto">
  ...
  // ── No squads assigned ──────────────────────────────────────────────────
  if (!demoActive && !effectiveAdminAccess && mySquads.length === 0) {
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-6">
        <div className="w-full max-w-sm text-center space-y-4">
  ...
  if (!demoActive && needAreaSelection) {
    return (
      <AreaSelectScreen
        areas={myAreas}
        ...
        clubBrand={demoActive ? DEMO_CLUB_BRAND : resolveInstitutionBrand(institutionProfile, activeSquad)}
      />
    );
  }

  // ── Squad selection required ────────────────────────────────────────────
  if (!demoActive && needSquadSelection) {
    return (
      <SquadSelectModal
        squads={mySquads}
        userName={user?.full_name || ""}
  ...
  return (
    <WorkspaceContext.Provider value={{
      ...
      debugInfo: {
        ...
        institution_modules: institutionModules.map(row => ({ module_id: row.module_id, enabled: row.enabled !== false })),
