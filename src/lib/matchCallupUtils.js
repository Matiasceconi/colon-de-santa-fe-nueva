...
function isMembershipActiveOnDate(membership, matchDate) {
  if (!isActiveMembership(membership)) return false;
  if (!matchDate) return true;
  if (membership?.effective_from && String(membership.effective_from) > String(matchDate)) return false;
  if (membership?.effective_to && String(membership.effective_to) < String(matchDate)) return false;
  return true;
}
...
export async function loadMatchCallupState(match, fallbackPlayers = []) {
  ...
  (memberships || []).filter((membership) => isMembershipActiveOnDate(membership, match.date)).forEach((membership) => {
    const previous = membershipByPlayer.get(membership.player_id);
    if (!previous || membership.squad_id === match.squad_id || (previous.squad_id !== match.squad_id && String(membership.effective_from || "") > String(previous.effective_from || ""))) {
      membershipByPlayer.set(membership.player_id, membership);
    }
  });

  const allPlayers = (loadedPlayers || []).filter((player) => player.active !== false).map((player) => {
  ...
  allPlayers.forEach((player) => {
    const key = normalizeText(getPlayerName(player));
    if (!key) return;
    if (!playerByName.has(key)) playerByName.set(key, []);
    playerByName.get(key).push(player);
  });
  const rawCallups = (callups || []).filter((callup) => callup.status !== "desconvocado" && callup.callup_status !== "desconvocado").map((callup) => {
    if (playerMap.has(callup.player_id)) return callup;
    const candidates = playerByName.get(normalizeText(callup.player_name)) || [];
    if (candidates.length !== 1) return callup;
    return { ...callup, legacy_player_id: callup.player_id, player_id: candidates[0].id, player_name: getPlayerName(candidates[0]), identity_relinked_in_memory: true };
  });
  // Desduplicar por jugador. Prioriza una clave canónica del partido y luego rol/actualización.
  const rolePriority = { titular: 3, suplente: 2, pendiente: 1 };
  const callupByPlayer = new Map();
  rawCallups.forEach((callup) => {
    if (!callup.player_id) return;
    const prev = callupByPlayer.get(callup.player_id);
    const canonical = callup.callup_key === `${match.id}:${callup.player_id}` ? 1 : 0;
    const prevCanonical = prev?.callup_key === `${match.id}:${prev?.player_id}` ? 1 : 0;
    const updatedAt = Number(new Date(callup.updated_at || callup.updated_date || callup.created_date || 0));
    const prevUpdatedAt = Number(new Date(prev?.updated_at || prev?.updated_date || prev?.created_date || 0));
    const roleScore = rolePriority[callup.lineup_role] || 0;
    const prevRoleScore = rolePriority[prev?.lineup_role] || 0;
    if (!prev || canonical > prevCanonical || (canonical === prevCanonical && roleScore > prevRoleScore) || (canonical === prevCanonical && roleScore === prevRoleScore && updatedAt > prevUpdatedAt)) {
      callupByPlayer.set(callup.player_id, callup);
    }
  });
  const savedCallups = Array.from(callupByPlayer.values());
