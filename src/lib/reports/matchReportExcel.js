...
const DEFAULT_BRAND = { name: "Club" };

export function exportMatchReportExcel({ reportData, reportMeta, staffComment, reportConfig, clubBrand }) {
  const brandName = clubBrand?.name || DEFAULT_BRAND.name;
  ...
  const rows = [
    ...
    [brandName, reportData?.player?.squad_name || reportMeta?.squadName || ""],
