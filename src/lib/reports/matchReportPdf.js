...
// Identidad institucional por defecto (respaldo neutro). La pantalla que invoca
// el PDF debe pasar clubBrand desde useWorkspace() para que la marca sea dinámica.
const DEFAULT_BRAND = {
  name: "Club",
  shortName: "CLUB",
  logoUrl: "",
  colors: { ink: "#111827", greenDeep: "#0F172A" },
};

// Marca activa para funciones internas que no reciben brand por parámetro.
// Se actualiza al inicio de exportMatchReportPdf desde clubBrand.
const documentBrands = new WeakMap();
...
function drawHeader(doc, reportData, reportMeta, brand) {
  const { selected } = reportData;
  const clubName = brand?.name || DEFAULT_BRAND.name;
  const clubLogo = brand?.logoUrl || DEFAULT_BRAND.logoUrl;
  ...
  doc.setFontSize(10);
  doc.text(`PerformancePitch · ${clubName}`, MARGIN, 26);
  doc.setFontSize(9);
  ...
  try {
    if (clubLogo) doc.addImage(clubLogo, "PNG", PAGE_W - 34, 6, 22, 26);
  } catch {}
}
...
function drawPlayerBlock(doc, reportData) {
  ...
  setColor(doc, "setTextColor", getDocumentBrand(doc).colors.ink);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(15);
...
function drawMatchBlock(doc, matchData, y, contentW, showZoneChart = true) {
  ...
  // Section titles
  setColor(doc, "setTextColor", getDocumentBrand(doc).colors.greenDeep);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
...
function drawFooter(doc, brand) {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7);
    doc.text(`PerformancePitch · ${clubName} · ${new Date().toLocaleDateString("es-AR")}`, MARGIN, PAGE_H - 6);
    doc.text(`Página ${i} de ${total}`, PAGE_W - MARGIN, PAGE_H - 6, { align: "right" });
  }
}

export async function exportMatchReportPdf({ reportData, reportMeta, staffComment, reportConfig, clubBrand }) {
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
  const brand = clubBrand || DEFAULT_BRAND;

  const config = normalizeMatchReportConfig(reportConfig || reportData?.reportConfig);
  documentBrands.set(doc, brand);
  drawHeader(doc, reportData, reportMeta, brand);
