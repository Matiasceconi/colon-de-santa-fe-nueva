...
const DEFAULT_BRAND = {
  ...
  logoUrl: "",
  colors: {
    green: "#1E293B", greenDark: "#0F172A", greenDeep: "#0F172A",
    yellow: "#0EA5E9", yellowDark: "#0284C7", gold: "#0EA5E9",
    ink: "#111827", muted: "#6B7280", panel: "#F8FAFC", line: "#E2E8F0",
  },
};
...
function drawHeader(doc, { logo, title, subtitle, dateLabel, pageW, margin }) {
  ...
  setColor(doc, "setFillColor", getDocumentBrand(doc).colors.panel);
  doc.roundedRect(margin, margin, contentW, 34, 4, 4, "F");
  setColor(doc, "setFillColor", getDocumentBrand(doc).colors.green);
  doc.rect(margin, margin + 32, contentW, 2, "F");
  if (logo) doc.addImage(logo, "PNG", margin + 5, margin + 5, 22, 22);
  setColor(doc, "setTextColor", getDocumentBrand(doc).colors.greenDark);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.text(getDocumentBrand(doc).name.toUpperCase(), margin + 32, margin + 9);
  setColor(doc, "setTextColor", getDocumentBrand(doc).colors.ink);
  doc.setFontSize(18);
  doc.text(title, margin + 32, margin + 19);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  setColor(doc, "setTextColor", getDocumentBrand(doc).colors.muted);
...
function drawSmallHeader(doc, { title, page, total, pageW, margin }) {
  ...
  setColor(doc, "setDrawColor", getDocumentBrand(doc).colors.line);
  doc.line(margin, margin + 6, pageW - margin, margin + 6);
}

function drawTableHeader(doc, { cols, y, margin, contentW }) {
  setColor(doc, "setFillColor", getDocumentBrand(doc).colors.greenDark);
...
export async function exportSkinfoldPdf({
  ...
  documentBrands.set(doc, { ...DEFAULT_BRAND, ...clubBrand, colors: { ...DEFAULT_BRAND.colors, ...clubBrand?.colors } });
