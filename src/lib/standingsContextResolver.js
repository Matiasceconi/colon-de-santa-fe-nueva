...
export function resolveActiveStandingsContext({ activeSquad, activeSeasonId, internalCompetitions = [], apiCompetitions = [] }) {
  ...
  // 1. Buscar competencia interna por squad_id o squad_ids (multi-selección)
  let internalComp = (internalCompetitions || []).find(
    (c) => c.active !== false && (c.squad_id === activeSquad?.id || (Array.isArray(c.squad_ids) && c.squad_ids.includes(activeSquad?.id)))
  );
