  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-500">
        {!plan.is_template && <span className="flex items-center gap-1"><Users size={11} /> {plan.player_name || plan.assignments?.[0]?.player_name || 'Sin jugador'}</span>}
...
export default function ComplementaryStrengthPlans() {
  ...
  function duplicatePlan(plan) {
    const sourceIsTemplate = !!plan.is_template;
    const copy = { ...plan, id: undefined, name: `${plan.name} (copia)`, status: 'draft', is_template: false, player_id: sourceIsTemplate ? '' : (plan.player_id || plan.assignments?.[0]?.player_id || ''), player_name: sourceIsTemplate ? '' : (plan.player_name || plan.assignments?.[0]?.player_name || ''), assignments: sourceIsTemplate ? [] : (plan.assignments || []).slice(0, 1).map((a) => ({ player_id: a.player_id, player_name: a.player_name })), workouts: (plan.workouts || []).map((w) => ({ ...w, id: undefined, blocks: (w.blocks || []).map((b) => ({ ...b, id: undefined, exercises: (b.exercises || []).map((e) => ({ ...e, id: undefined, overrides: [] })) })) })) };
    setEditingPlan(copy);
    setBuilderOpen(true);
    setTab('create');
  }
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2"><Dumbbell size={20} className="text-blue-400" /> Planes individuales</h1>
          <p className="text-xs text-zinc-500 mt-1">El PF prescribe un plan para un único jugador y lo publica directamente en su portal · {activeSquad?.name}</p>
        </div>
      </div>
      ...
      {tab === 'create' && (
        builderOpen ? (
          ...
          <div className="space-y-4">
            <button onClick={() => { setEditingPlan(null); setBuilderOpen(true); }} className="flex items-center gap-2 px-4 py-2.5 bg-white text-zinc-950 rounded-xl text-sm font-bold hover:bg-zinc-200"><Plus size={16} /> Crear plan individual</button>
            {draftPlans.length > 0 && (
