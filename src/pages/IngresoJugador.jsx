      ...
      <div className="min-h-screen flex bg-zinc-950">
        <div className="hidden lg:flex lg:w-1/2 flex-col items-center justify-center p-12 bg-zinc-900 border-r border-zinc-800 relative overflow-hidden">
          ...
          <div className="relative z-10 text-center max-w-sm">
            <div className="flex items-center justify-center w-20 h-20 rounded-2xl bg-zinc-800 border border-zinc-700 mb-8 mx-auto overflow-hidden">{brand.logo_url ? <img src={brand.logo_url} alt={brand.club_name} className="w-full h-full object-contain p-2" /> : <Shield size={36} style={{ color: brand.accent_color }} />}</div>
            <h1 className="text-4xl font-black text-white tracking-tight mb-3">{brand.club_name}</h1>
            <p className="text-zinc-400 text-base leading-relaxed">Portal de jugadores</p>
          </div>
        </div>
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-sm">
            <div className="flex items-center gap-2 mb-8 lg:hidden">{brand.logo_url ? <img src={brand.logo_url} alt="" className="w-8 h-8 object-contain" /> : <Shield size={22} style={{ color: brand.accent_color }} />}<span className="text-lg font-black text-white">{brand.short_name || brand.club_name}</span></div>
            ...
            <form onSubmit={handleDniSubmit} className="space-y-5">
              ...
              <button type="submit" disabled={loading || !dni} style={{ backgroundColor: brand.primary_color }} className="w-full h-12 rounded-xl text-white font-bold disabled:opacity-50 flex items-center justify-center gap-2 transition-opacity hover:opacity-90">{loading ? <><Loader2 size={18} className="animate-spin" /> Verificando...</> : 'Continuar'}</button>
            </form>
            <div className="mt-6 rounded-xl border border-zinc-800 bg-zinc-900/70 p-4">
              <p className="text-xs font-bold uppercase tracking-wider text-zinc-400">Cómo usar el portal</p>
              <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                {[["1", "Ingresá tu DNI"], ["2", "Completá Wellness"], ["3", "Respondé el RPE"]].map(([number, label]) => <div key={number}><span className="mx-auto flex h-7 w-7 items-center justify-center rounded-full bg-zinc-800 text-xs font-black text-white">{number}</span><p className="mt-1.5 text-[10px] leading-4 text-zinc-500">{label}</p></div>)}
              </div>
              <p className="mt-3 text-center text-[10px] leading-4 text-zinc-600">También vas a ver el cronograma del día y los entrenamientos asignados.</p>
            </div>
            <Link to="/" className="mt-5 flex items-center justify-center gap-1.5 text-sm text-zinc-400 hover:text-white transition-colors"><ArrowLeft size={14} /> Volver</Link>
          </div>
        </div>
      </div>
    );
