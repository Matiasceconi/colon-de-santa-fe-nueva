...
import MatchSummaryTab from "@/components/matches/tabs/MatchSummaryTab";
import ConvocatoriaFormacionTab from "@/components/matches/tabs/ConvocatoriaFormacionTab";
...
import MatchTimelineTab from "@/components/matches/tabs/MatchTimelineTab";
import MatchStatisticsTab from "@/components/matches/tabs/MatchStatisticsTab";
...
const TAB_ALIASES = { resumen: "resumen", summary: "resumen", plan: "plan-video", "plan-video": "plan-video", "convocatoria-formacion": "convocatoria-formacion", convocatoria: "convocatoria-formacion", formacion: "convocatoria-formacion", formation: "convocatoria-formacion", convocados: "convocatoria-formacion", cronologia: "cronologia", timeline: "cronologia", estadisticas: "estadisticas", statistics: "estadisticas", minutos: "minutos", gps: "gps", logistica: "logistica" };
...
export default function MatchDetail() {
  ...
  const { toast } = useToast();
  const { activeSeasonId, mySquads, clubBrand } = useWorkspace();

  const [match, setMatch] = useState(null);
  const [players, setPlayers] = useState([]);
  const [competitions, setCompetitions] = useState([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState(() => TAB_ALIASES[new URLSearchParams(window.location.search).get("tab")] || "resumen");
    ...
    <div className="space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-4 rounded-2xl border border-zinc-800 bg-zinc-900/90 px-4 py-3 shadow-lg shadow-black/20">
                <div className="flex items-center gap-3">
                  <LogoAvatar src={clubBrand?.logoUrl} alt={clubBrand?.name || "Club"} club />
                  <div>
                    <p className="text-xs uppercase tracking-[0.24em] text-zinc-500">{clubBrand?.name || "Club"}</p>
                    <p className="text-xl font-black text-white">{clubBrand?.shortName || "CLUB"}</p>
                  </div>
                </div>
      ...
      <Tabs value={activeTab} onValueChange={changeTab} className="space-y-4">
        <TabsList className="h-auto flex w-full flex-wrap justify-start gap-2 rounded-2xl border border-zinc-800 bg-zinc-900 p-2">
          <TabsTrigger value="resumen" className="rounded-xl px-4 py-2 text-sm data-[state=active]:bg-yellow-500 data-[state=active]:text-zinc-950">Resumen</TabsTrigger>
          <TabsTrigger value="convocatoria-formacion" className="rounded-xl px-4 py-2 text-sm data-[state=active]:bg-yellow-500 data-[state=active]:text-zinc-950">Convocatoria y formación</TabsTrigger>
          <TabsTrigger value="cronologia" className="rounded-xl px-4 py-2 text-sm data-[state=active]:bg-yellow-500 data-[state=active]:text-zinc-950">Cronología</TabsTrigger>
          <TabsTrigger value="estadisticas" className="rounded-xl px-4 py-2 text-sm data-[state=active]:bg-yellow-500 data-[state=active]:text-zinc-950">Estadísticas</TabsTrigger>
        ...
        <TabsContent value="resumen" className="mt-0">
          <MatchSummaryTab match={match} players={players} onMatchUpdated={handleMatchUpdated} onNavigateTab={changeTab} />
        </TabsContent>
        <TabsContent value="convocatoria-formacion" className="mt-0">
          <ConvocatoriaFormacionTab match={match} players={players} refreshKey={callupsVersion} onMatchUpdated={handleMatchUpdated} onRegisterSave={setHeaderSave} onCallupsUpdated={() => setCallupsVersion((value) => value + 1)} />
        </TabsContent>
        <TabsContent value="cronologia" className="mt-0">
          <MatchTimelineTab match={match} players={players} />
        </TabsContent>
        <TabsContent value="estadisticas" className="mt-0">
          <MatchStatisticsTab match={match} />
        </TabsContent>
