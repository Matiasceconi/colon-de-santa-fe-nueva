...
import { Plus, Edit2, Trash2, Youtube, Users, X, Check, Upload, FileSpreadsheet, ExternalLink, Clock, Trophy, SlidersHorizontal, Sparkles, CalendarCheck2, PenLine } from "lucide-react";
import { useToast } from "@/components/ui/use-toast";
import { useWorkspace } from "@/lib/WorkspaceContext";
import { useDemo } from "@/lib/DemoContext";
import DemoMatches from "@/components/demo/DemoMatches";
...
import { normalizeShieldKey, resolveClubShield } from "@/lib/clubShields";
moment.locale("es");
...
// Resuelve el escudo desde el RivalClub vinculado o por nombre normalizado.
// Reemplaza al mapa hardcoded CLUB_LOGOS — la fuente canónica es RivalClub.shield_url.
function getLogoFromClubs(name, clubs) {
  if (!name || !clubs?.length) return null;
  const key = normalizeShieldKey(name);
  const byName = clubs.find((club) => (
    normalizeShieldKey(club.official_name) === key ||
    normalizeShieldKey(club.short_name) === key ||
    (club.aliases || []).some((alias) => normalizeShieldKey(alias) === key)
  ));
  return resolveClubShield(name, byName?.shield_url);
}

function cleanTeamName(name) {
  return String(name || "").replace(/\s*\((reserva|reserves)\)\s*/gi, " ").replace(/\s+/g, " ").trim();
}

function normalizeClubName(name) {
  return normalizeShieldKey(name)
    .replace(/\b(club|atletico|atletica|asociacion|ca)\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}

function sameClubTeam(name, aliases) {
  const key = normalizeClubName(name);
  return Boolean(key) && aliases.some((alias) => key === alias || (alias.length >= 5 && (key.includes(alias) || alias.includes(key))));
}

function categoryKey(value) {
  const normalized = normalizeStr(value);
  if (normalized.includes("reserva") || normalized.includes("proyeccion")) return "reserva";
  if (normalized.includes("primera") || normalized.includes("nacional")) return "primera";
  if (normalized.includes("cuarta") || normalized.includes("4ta")) return "cuarta";
  if (normalized.includes("quinta") || normalized.includes("5ta")) return "quinta";
  if (normalized.includes("sexta") || normalized.includes("6ta")) return "sexta";
  if (normalized.includes("septima") || normalized.includes("7ma")) return "septima";
  if (normalized.includes("octava") || normalized.includes("8va")) return "octava";
  if (normalized.includes("novena") || normalized.includes("9na")) return "novena";
  return normalized;
}

function competitionForSquad(competitions, squad) {
  if (!squad?.id) return null;
  const linked = competitions.find((competition) => (
    competition.squad_id === squad.id || (competition.squad_ids || []).includes(squad.id)
  ));
  if (linked) return linked;
  const key = categoryKey(squad.name);
  const division = key === "primera" ? "primera" : key === "reserva" ? "reserva" : "juveniles";
  return competitions.find((competition) => competition.division === division) || null;
}

function findRivalClub(name, clubs) {
  const key = normalizeShieldKey(name);
...
function ClubLogo({ url, name, size = 10 }) {
  ...
  const resolved = resolveClubShield(name, url);
  if (!resolved || err) {
    return (
  ...
  return <img src={resolved} alt={name} className={`${sizeClass} object-contain shrink-0`} onError={() => setErr(true)} />;
}

function CompetitionLogo({ competition, size = "h-9 w-9" }) {
  if (!competition?.logo) {
    return <span className={`${size} flex shrink-0 items-center justify-center rounded-lg border border-zinc-700 bg-zinc-800 text-zinc-500`}><Trophy size={15} /></span>;
  }
  return <img src={competition.logo} alt={competition.short_name || competition.name} className={`${size} shrink-0 rounded-lg bg-white object-contain p-1`} />;
}
...
// ── MatchCard ─────────────────────────────────────────────────────────────────
function MatchCard({ match, players, onEdit, onDelete, onMatchUpdated, squadId, competitions, competitionMap, rivalClubs, stats = {} }) {
  ...
  const clubLogo = clubBrand?.logoUrl || "";
  const clubName = clubBrand?.name || "Club";
  ...
  const rivalLogo = match.rival_logo_url || getLogoFromClubs(match.rival, rivalClubs);
  const leftLogo = isLocal ? clubLogo : rivalLogo;
  const rightLogo = isLocal ? rivalLogo : clubLogo;
  const leftName = isLocal ? clubName : match.rival;
  const rightName = isLocal ? match.rival : clubName;
  const leftScore = isLocal ? match.our_score : match.rival_score;
  const rightScore = isLocal ? match.rival_score : match.our_score;
  const isFuture = String(match.date || '') >= moment().format('YYYY-MM-DD') && !hasResult && match.status !== 'finalizado';
  const prepChecks = isFuture ? [
    stats.callupCount > 0,
    stats.titularCount === 11,
    stats.missingShirts === 0 && stats.callupCount > 0,
    !!match.captain_player_id,
    !!(match.match_video_url || match.match_plan_pdf_url || match.video_analysis_url),
    !!match.match_logistics,
    !!stats.equipmentReady,
  ] : [];
  const prepPct = prepChecks.length ? Math.round((prepChecks.filter(Boolean).length / prepChecks.length) * 100) : 0;
  const detailBadges = [
    isFuture ? { label: `Preparación ${prepPct}%`, className: prepPct >= 80 ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-300" : "bg-blue-500/10 border-blue-500/20 text-blue-300" } : null,
    stats.callupCount > 0 ? { label: `Convocatoria ${stats.callupCount}`, className: "bg-emerald-500/10 border-emerald-500/20 text-emerald-300" } : null,
    stats.titularCount > 0 ? { label: `Titulares ${stats.titularCount}/11`, className: stats.titularCount === 11 ? "bg-blue-500/10 border-blue-500/20 text-blue-300" : "bg-amber-500/10 border-amber-500/20 text-amber-300" } : null,
    stats.officialMinutesCount > 0 ? { label: `Minutos ${stats.officialMinutesCount}`, className: "bg-yellow-500/10 border-yellow-500/20 text-yellow-300" } : null,
    stats.gpsCount > 0 || match.csv_url ? { label: `GPS${stats.gpsCount ? ` ${stats.gpsCount}` : ""}`, className: "bg-green-500/10 border-green-500/20 text-green-300" } : null,
    stats.equipmentReady ? { label: "Utilería lista", className: "bg-orange-500/10 border-orange-500/20 text-orange-300" } : null,
    ...
    stats.duplicateCallups > 0 ? { label: `⚠ ${stats.duplicateCallups} duplicados ocultos`, className: "bg-red-500/10 border-red-500/20 text-red-300" } : null,
  ].filter(Boolean);
  ...
  return (
    <div className={`bg-gradient-to-r from-zinc-900 to-zinc-900/80 border border-zinc-800 border-l-4 ${borderClass} rounded-xl overflow-hidden shadow-lg shadow-black/20`}>
      <div className="grid grid-cols-1 lg:grid-cols-[88px_1fr_100px_1fr_210px_94px] items-stretch cursor-pointer" onClick={() => { sessionStorage.setItem("matches_scroll", String(window.scrollY || 0)); navigate(`/matches/${match.id}?tab=resumen`); }}>
...
// ── MatchForm ─────────────────────────────────────────────────────────────────
const EMPTY = {
  squad_id: "", squad_name: "", season_id: "", calendar_event_id: "", upcoming_match_id: "", external_match_key: "", integration_source: "",
  date: "", match_time: "", match_venue: "", rival: "", rival_club_id: "", competition: "", competition_id: "", competition_stage: "", competition_round: "", group_name: "", matchday_number: "", phase_label: "", location: "Local",
...
function MatchForm({ initial, onSave, onCancel, competitions, squads, activeSquad, activeSeasonId, clubs, onClubCreated }) {
  ...
  const currentSquad = squads.find((squad) => squad.id === form.squad_id) || activeSquad;
  const linkedCompetitions = competitions.filter((competition) => (
    competition.squad_id === form.squad_id || (competition.squad_ids || []).includes(form.squad_id)
  ));
  const availableCompetitions = linkedCompetitions.length ? linkedCompetitions : competitions.filter((competition) => {
    const key = categoryKey(currentSquad?.name);
    const division = key === "primera" ? "primera" : key === "reserva" ? "reserva" : "juveniles";
    return competition.division === division;
  });
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-5 space-y-4">
      ...
      {form.upcoming_match_id && (
        <div className="flex items-start gap-3 rounded-xl border border-blue-500/30 bg-blue-500/10 px-4 py-3">
          <Sparkles size={17} className="mt-0.5 shrink-0 text-blue-300" />
          <div>
            <p className="text-sm font-semibold text-blue-200">Datos completados desde el Centro de Competencias</p>
            <p className="mt-0.5 text-xs text-blue-300/70">Revisá horario y sede; al guardar quedará vinculado al partido integrado y no podrá duplicarse.</p>
          </div>
        </div>
      )}
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
          ...
          <select className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-zinc-500" value={form.squad_id || ""} onChange={e => {
            ...
            const competition = competitionForSquad(competitions, squad);
            setForm((current) => ({
              ...current,
              squad_id: squad?.id || "",
              squad_name: squad?.name || "",
              season_id: squad?.season || current.season_id || "",
              competition_id: competition?.id || "",
              competition: competition?.name || "",
              competition_stage: competition?.phase_config?.[0]?.name || "",
            }));
          }}>
        ...
        <div className="sm:col-span-2">
          <label className="text-xs text-zinc-400 mb-1.5 block">Competencia</label>
          <div className="grid gap-2 sm:grid-cols-3">
            {availableCompetitions.map((option) => {
              const selected = option.id === form.competition_id;
              return (
                <button
                  type="button"
                  key={option.id}
                  onClick={() => setCompetition(option.id)}
                  className={`flex items-center gap-3 rounded-xl border px-3 py-2.5 text-left transition ${selected ? "border-yellow-500/50 bg-yellow-500/10 text-white" : "border-zinc-700 bg-zinc-800 text-zinc-300 hover:border-zinc-600"}`}
                >
                  <CompetitionLogo competition={option} />
                  <span className="min-w-0">
                    <span className="block truncate text-xs font-bold">{option.short_name || option.name}</span>
                    <span className="mt-0.5 block text-[10px] uppercase tracking-wide text-zinc-500">{option.division || "competencia"}</span>
                  </span>
                  {selected && <Check size={14} className="ml-auto shrink-0 text-yellow-300" />}
                </button>
              );
            })}
            {availableCompetitions.length === 0 && <p className="rounded-xl border border-dashed border-zinc-700 px-3 py-4 text-xs text-zinc-500">No hay una competencia vinculada a este plantel.</p>}
          </div>
        </div>
        <div>
...
// ── Main ──────────────────────────────────────────────────────────────────────
function OperationalMatches() {
  const { activeSquadId, activeSquad, activeSeasonId, mySquads, institutionProfile, clubBrand } = useWorkspace();
  ...
  const [matchGpsRows, setMatchGpsRows] = useState([]);
  ...
  const clubAliases = useMemo(() => [...new Set([
    institutionProfile?.official_name,
    institutionProfile?.short_name,
    institutionProfile?.abbreviation,
    clubBrand?.name,
    clubBrand?.shortName,
  ].map(normalizeClubName).filter((value) => value && value.length >= 3))], [institutionProfile, clubBrand]);
  const matchStats = useMemo(() => {
    const byMatch = {};
    const get = (id) => {
      if (!byMatch[id]) byMatch[id] = { callupCount: 0, titularCount: 0, missingShirts: 0, officialMinutesCount: 0, gpsCount: 0, equipmentReady: false, duplicateCallups: 0 };
      return byMatch[id];
    };
    const callupsByMatch = {};
    matchCallups.forEach((row) => {
      if (!row.match_id || row.status === "desconvocado" || row.callup_status === "desconvocado") return;
      if (!callupsByMatch[row.match_id]) callupsByMatch[row.match_id] = [];
      callupsByMatch[row.match_id].push(row);
    });
    Object.entries(callupsByMatch).forEach(([matchId, rows]) => {
      const rank = { titular: 3, suplente: 2, pendiente: 1 };
      const unique = new Map();
      rows.forEach((row) => {
        if (!row.player_id) return;
        const previous = unique.get(row.player_id);
        if (!previous || (rank[row.lineup_role] || 0) > (rank[previous.lineup_role] || 0)) unique.set(row.player_id, row);
      });
      const stats = get(matchId);
      const values = Array.from(unique.values());
      stats.callupCount = values.length;
      stats.titularCount = values.filter((row) => row.lineup_role === "titular").length;
      stats.missingShirts = values.filter((row) => !Number(row.shirt_number)).length;
      stats.duplicateCallups = Math.max(0, rows.length - values.length);
    });
    ...
    matchMinutes.forEach((row) => {
      ...
      if (!officialByMatch[row.match_id]) officialByMatch[row.match_id] = new Set();
      officialByMatch[row.match_id].add(row.player_id);
    });
    Object.entries(officialByMatch).forEach(([matchId, ids]) => { get(matchId).officialMinutesCount = ids.size; });
    const gpsByMatch = {};
    matchGpsRows.forEach((row) => {
      if (!row.session_id) return;
      if (!gpsByMatch[row.session_id]) gpsByMatch[row.session_id] = new Set();
      if (row.player_id) gpsByMatch[row.session_id].add(row.player_id);
    });
    Object.entries(gpsByMatch).forEach(([matchId, ids]) => { get(matchId).gpsCount = ids.size; });
    equipmentChecklists.forEach((checklist) => {
      if (!checklist.match_id || checklist.context_type !== "match") return;
      const ready = ["prepared", "checked", "closed"].includes(checklist.status) && (checklist.items || []).every((item) => Number(item.prepared_qty || 0) >= Number(item.required_qty || 0));
      if (ready) get(checklist.match_id).equipmentReady = true;
    });
    return byMatch;
  }, [matchCallups, matchMinutes, matchGpsRows, equipmentChecklists]);
  ...
  const integratedCandidates = useMemo(() => {
    const squadKey = categoryKey(activeSquad?.name);
    const today = new Date().toISOString().slice(0, 10);
    return upcomingMatches
      .filter((match) => (
        match.status === "scheduled" &&
        match.matchDate >= today &&
        categoryKey(`${match.category || ""} ${match.competition || ""}`) === squadKey &&
        (sameClubTeam(match.homeTeam, clubAliases) || sameClubTeam(match.awayTeam, clubAliases)) &&
        !matches.some((existing) => (
          (match.external_key && existing.external_match_key === match.external_key) ||
          existing.upcoming_match_id === match.id ||
          (
            existing.date === match.matchDate &&
            normalizeShieldKey(existing.rival) === normalizeShieldKey(sameClubTeam(match.homeTeam, clubAliases) ? match.awayTeam : match.homeTeam)
          )
        ))
      ))
      .sort((a, b) => String(a.matchDate).localeCompare(String(b.matchDate)))
      .slice(0, 6);
  }, [upcomingMatches, activeSquad?.name, clubAliases, matches]);
  ...
  async function loadAll() {
    ...
    try {
      ...
      const all = await base44.entities.MatchReport.list("-date", 500);
      const p = await base44.entities.Player.list("-created_date", 1200);
      const competitionRows = await base44.entities.Competitions.list("name", 300);
      const clubRows = await base44.entities.RivalClub.list("official_name", 500).catch(() => []);
      const eventRows = await base44.entities.DayEvent.list("date", 1000).catch(() => []);
      const integratedRows = await base44.entities.UpcomingMatch.list("matchDate", 2500).catch(() => []);
      const callupRows = await base44.entities.MatchCallup.list("-updated_date", 3000).catch(() => []);
      const minuteRows = await base44.entities.MatchPlayerMinutes.list("-updated_date", 3000).catch(() => []);
      const gpsRows = await base44.entities.CatapultReport.list("-date", 3000).catch(() => []);
      const checklistRows = await base44.entities.EquipmentChecklist.list("-event_date", 1000).catch(() => []);
      ...
      setUpcomingMatches(integratedRows);
      setMatchCallups(callupRows);
      setMatchMinutes(minuteRows);
      setMatchGpsRows(gpsRows);
      setEquipmentChecklists(checklistRows);
    } catch (error) {
      console.error("Matches loadAll error:", error);
      toast({ title: "No se pudo cargar la lista de partidos", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function save(form) {
    if (!editing?.id && (form.upcoming_match_id || form.external_match_key)) {
      const repeatedLocally = matches.some((match) => (
        (form.external_match_key && match.external_match_key === form.external_match_key) ||
        match.upcoming_match_id === form.upcoming_match_id ||
        (match.date === form.date && normalizeShieldKey(match.rival) === normalizeShieldKey(form.rival))
      ));
      const repeatedInDatabase = form.external_match_key
        ? await base44.entities.MatchReport.filter({ external_match_key: form.external_match_key }, "-created_date", 1).catch(() => [])
        : await base44.entities.MatchReport.filter({ upcoming_match_id: form.upcoming_match_id }, "-created_date", 1).catch(() => []);
      if (repeatedLocally || repeatedInDatabase.length) {
        toast({ title: "Ese partido ya fue creado", description: "La integración está vinculada a un partido operativo existente.", variant: "destructive" });
        return;
      }
    }
    const selectedCompetition = competitions.find((competition) => competition.id === form.competition_id);
    ...
    base44.functions.invoke('manageDailySchedule', { action: 'sync', squad_id: data.squad_id, squad_name: data.squad_name || '', season_id: data.season_id || '', from: data.date, to: data.date }).catch(() => {});
    setShowForm(false);
    setEditing(null);
    loadAll();
  }

  function selectIntegratedMatch(match) {
    const isHome = sameClubTeam(match.homeTeam, clubAliases);
    const rawRival = isHome ? match.awayTeam : match.homeTeam;
    const rival = cleanTeamName(rawRival);
    const rivalClub = findRivalClub(rival, rivalClubs);
    const competition = competitionForSquad(competitions, activeSquad);
    const roundNumber = Number(String(match.round || "").match(/\d+/)?.[0] || "") || "";
    const directLogo = isHome ? match.awayLogo : match.homeLogo;
    setEditing({
      ...EMPTY,
      squad_id: activeSquad?.id || activeSquadId || "",
      squad_name: activeSquad?.name || "",
      season_id: match.season || activeSeasonId || "",
      date: match.matchDate || "",
      match_time: match.matchTime || "",
      match_venue: match.venue || "",
      rival,
      rival_club_id: rivalClub?.id || "",
      rival_logo_url: resolveClubShield(rawRival, rivalClub?.shield_url || directLogo),
      competition: competition?.name || match.competition || "",
      competition_id: competition?.id || "",
      competition_stage: competition?.phase_config?.[0]?.name || "Fase regular",
      competition_round: match.round || "",
      matchday_number: roundNumber,
      phase_label: match.round || "",
      location: isHome ? "Local" : "Visitante",
      upcoming_match_id: match.id,
      external_match_key: match.external_key || `${match.source || "integracion"}:${match.category}:${match.matchDate}:${normalizeShieldKey(match.homeTeam)}:${normalizeShieldKey(match.awayTeam)}`,
      integration_source: match.source || "Centro de Competencias",
    });
    ...
    <div className="space-y-4">
      ...
      {newChoiceOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-zinc-700 bg-zinc-950 p-5 shadow-2xl">
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-white">Nuevo partido</h2>
                <p className="mt-0.5 text-sm text-zinc-500">Elegí el origen. La categoría activa es <span className="font-semibold text-zinc-300">{activeSquad?.name || "sin definir"}</span>.</p>
              </div>
            ...
            <div className="space-y-3">
              <div className="rounded-xl border border-blue-500/35 bg-blue-500/10 p-4">
                <div className="mb-3 flex items-center gap-2">
                  <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-500/15 text-blue-300"><Sparkles size={16} /></span>
                  <div>
                    <p className="text-sm font-semibold text-blue-200">Crear desde la integración</p>
                    <p className="text-xs text-blue-300/60">Próximo fixture oficial del Centro de Competencias</p>
                  </div>
                  <span className="ml-auto rounded-full border border-blue-400/20 bg-blue-400/10 px-2 py-1 text-[9px] font-black uppercase tracking-wide text-blue-300">Recomendado</span>
                </div>
                {integratedCandidates.length === 0 ? (
                  <div className="rounded-lg border border-dashed border-blue-400/20 px-3 py-4 text-sm text-zinc-500">
                    No hay un próximo partido integrado pendiente para este plantel. Puede estar ya creado o todavía no confirmado.
                  </div>
                ) : (
                  <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
                    {integratedCandidates.map((match, index) => {
                      const competition = competitionForSquad(competitions, activeSquad);
                      return (
                        <button key={match.id} onClick={() => selectIntegratedMatch(match)} className="w-full rounded-xl border border-zinc-800 bg-zinc-950/70 p-3 text-left transition hover:border-blue-400/45 hover:bg-zinc-900">
                          <div className="flex items-center gap-3">
                            <CompetitionLogo competition={competition} size="h-10 w-10" />
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-2">
                                <p className="truncate text-sm font-black text-white">{match.homeTeam} vs {match.awayTeam}</p>
                                {index === 0 && <span className="shrink-0 rounded bg-emerald-500/15 px-1.5 py-0.5 text-[9px] font-bold text-emerald-300">PRÓXIMO</span>}
                              </div>
                              <p className="mt-1 text-xs text-zinc-500">{moment(match.matchDate).format("DD/MM/YYYY")} · {match.matchTime || "Horario por confirmar"} · {match.round || match.competition}</p>
                              <p className="mt-0.5 truncate text-[10px] text-zinc-600">{match.venue || "Sede por confirmar"} · {match.source || "Fuente integrada"}</p>
                            </div>
                            <div className="flex shrink-0 items-center gap-1.5">
                              <ClubLogo name={match.homeTeam} url={match.homeLogo} size={8} />
                              <span className="text-[10px] font-black text-zinc-600">VS</span>
                              <ClubLogo name={match.awayTeam} url={match.awayLogo} size={8} />
                            </div>
                          </div>
                        </button>
                      );
              ...
              <div className="rounded-xl border border-zinc-700 bg-zinc-900 p-4">
                <div className="mb-3 flex items-center gap-2">
                  <CalendarCheck2 size={16} className="text-yellow-300" />
                  <p className="text-sm font-semibold text-zinc-200">Usar un evento de Calendario</p>
                </div>
                {availableCalendarMatches.length === 0 ? <p className="text-sm text-zinc-500">No hay eventos próximos de tipo Partido sin vincular.</p> : <div className="max-h-48 space-y-2 overflow-y-auto">{availableCalendarMatches.map((event) => <button key={event.id} onClick={() => selectCalendarEvent(event)} className="flex w-full items-center justify-between gap-3 rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2 text-left hover:border-yellow-500/40"><div><p className="text-sm font-semibold text-white">{event.rival || event.title || "Partido"}</p><p className="text-xs text-zinc-500">{moment(event.date).format("DD/MM/YYYY")} · {event.time || event.start_time || "Sin horario"} · {event.competition || "Sin competencia"}</p></div>{event.rival_logo_url && <ClubLogo name={event.rival} url={event.rival_logo_url} />}</button>)}</div>}
              </div>

              <button onClick={createManualMatch} className="flex w-full items-center gap-3 rounded-xl border border-zinc-700 bg-zinc-900 px-4 py-3 text-left text-sm text-zinc-200 transition hover:bg-zinc-800">
                <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-zinc-800 text-zinc-400"><PenLine size={15} /></span>
                <span><span className="block font-semibold">Crear partido manualmente</span><span className="mt-0.5 block text-xs text-zinc-500">Elegí un club con escudo y una competencia ya configurada.</span></span>
              </button>
            </div>
          </div>
        </div>
      ...
      {visibleMatches.length === 0 && !showForm ? (
        ...
        <div className="space-y-3">
          {visibleMatches.map(m => (
            <MatchCard key={m.id} match={m} players={players} onEdit={m2 => { setEditing(m2); setShowForm(true); }} onDelete={remove} onMatchUpdated={handleMatchUpdated} squadId={activeSquadId} competitions={competitions} competitionMap={competitionMap} rivalClubs={rivalClubs} />
          ))}
        </div>
      )}
    </div>
...
export default function Matches() {
  ...
  return demoActive ? <DemoMatches /> : <OperationalMatches />;
}