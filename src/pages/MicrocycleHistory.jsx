...
import { Copy, Download, FileText, GitCompare, Search, Tag, X } from "lucide-react";
...
export default function MicrocycleHistory() {
  const { activeSquadId, clubBrand, can } = useWorkspace();
  ...
  async function exportPdf(summary) {
    try {
    assertExportAllowed(can, "/performance/microcycle-history");
    if (summary.pdf_url) { window.open(summary.pdf_url, "_blank"); return; }
    const pdf = await createBrandedPdf({brand:clubBrand,title:displayName(summary),subtitle:dateRange(summary),meta:[summary.squad_name,summary.rival].filter(Boolean)});
    pdf.sectionTitle("Carga del microciclo");
    pdf.table({columns:[{key:"label",label:"Métrica"},{key:"value",label:"Valor",align:"right"}],rows:COMPARE_KEYS.map(key=>{
      const metric=MICRO_METRICS.find(m=>m.key===key);
      const value=summary.gps_variables_snapshot?.[key]?.value ?? summary.load_summary_snapshot?.[key]?.value ?? summary.snapshot?.loadSummary?.[key]?.value;
      return {label:metric?.label||key,value:value==null?"—":fmt(value,metric?.unit)};
    })});
    pdf.sectionTitle("Sesiones del microciclo");
    pdf.table({columns:[{key:"date",label:"Fecha"},{key:"title",label:"Sesión",width:2},{key:"session_type",label:"Tipo"}],rows:summary.sessions_snapshot||[]});
    const doc=pdf.finalize();
    const fileName=buildExportFileName("Microciclo",[clubBrand?.name,summary.squad_name,summary.microcycle_number,summary.fecha_inicio]);
    doc.save(fileName);
    } catch(error) { toast({title:"No se pudo exportar el microciclo",description:error.message,variant:"destructive"}); }
  }
