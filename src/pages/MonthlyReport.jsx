...
import { FileDown, Loader2, BarChart2, Clock, Zap } from "lucide-react";
...
// ── PDF Generator ─────────────────────────────────────────────────────────────
async function generatePDF(month, year, playerStats, sessionCount, clubName) {
  ...
  doc.text(`${clubName || "Club"} — Cuerpo Técnico`, MARGIN, 20);
...
export default function MonthlyReport() {
  ...
  async function handleExport() {
    ...
    setGenerating(true);
    await generatePDF(month, year, playerStats, monthSessions.length, clubBrand?.name);
    setGenerating(false);
  }
