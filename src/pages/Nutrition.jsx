...
import { CheckCircle2, Clock3, FileSpreadsheet, LayoutDashboard, RefreshCw, Table2 } from "lucide-react";
...
export default function Nutrition() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [assessmentRows, interpretationRows, playerRows, membershipRows, syncRows, squadRows, readingStatusRows] = await Promise.all([
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] space-y-5 bg-zinc-950 p-4 text-white md:p-6">
      <header className="rounded-2xl border border-zinc-800 bg-gradient-to-br from-zinc-900 to-zinc-950 p-5 md:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold tracking-tight">Nutrición · {dataSquad?.name || activeSquad?.name || "Plantel"}</h1>
            <p className="mt-1 text-sm text-zinc-500">Modo híbrido: carga manual dentro del sistema + integración externa opcional {activeSeasonId ? `· Temporada ${activeSeasonId}` : ""}</p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            {canSync && <button onClick={handleSync} disabled={syncing} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white transition hover:bg-emerald-500 disabled:opacity-60"><RefreshCw size={16} className={syncing ? "animate-spin" : ""} />{syncing ? "Actualizando..." : "Actualizar integración"}</button>}
          </div>
        </div>
        <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-zinc-800 pt-4 text-xs text-zinc-500">
          <span className="rounded-full border border-emerald-500/20 bg-emerald-500/5 px-3 py-1.5 text-emerald-300">Carga manual disponible</span>
          <span className="rounded-full border border-zinc-800 bg-zinc-950 px-3 py-1.5">Integración actual: Antros grupales.xlsx</span>
      ...
      <div className="flex w-fit gap-1 rounded-xl border border-zinc-800 bg-zinc-900 p-1">
        <button onClick={() => setActiveView("dashboard")} className={`flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-semibold transition ${activeView === "dashboard" ? "bg-white text-zinc-900" : "text-zinc-400 hover:text-white"}`}><LayoutDashboard size={14} /> Resumen y evolución</button>
        <button onClick={() => setActiveView("manual")} className={`flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-semibold transition ${activeView === "manual" ? "bg-white text-zinc-900" : "text-zinc-400 hover:text-white"}`}><Table2 size={14} /> Planilla manual</button>
      </div>

      {activeView === "dashboard" ? (
        <NutritionDashboard players={players} assessments={assessments} interpretations={interpretations} activeSquad={dataSquad || activeSquad} />
      ) : (
        <NutritionManualSheet players={players} assessments={assessments} interpretations={interpretations} readingStatuses={readingStatuses} activeSquad={dataSquad || activeSquad} onReload={load} />
      )}
    </div>
  );
}
