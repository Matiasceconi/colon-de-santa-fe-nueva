...
import { SESSIONS_LIST_TOUR, SESSIONS_CREATE_TOUR, SESSIONS_DETAIL_TOUR } from "@/lib/pageTours";
...
export default function Sessions() {
  ...
  useEffect(() => {
    if (demoActive) { setLoading(false); return; }
    setLoading(true);
    setView("list");
    ...
    Promise.all([
      ...
      base44.entities.SessionPlayer.list("-created_date", 5000).catch(() => []),
    ]).then(([all, allExercises, allVideoLinks, allPlans, allPlanDays, allObjectives, allSessionPlayers]) => {
      ...
      allSessionPlayers.forEach(sp => {
        if (!["ausente", "no_entrena"].includes(sp.attendance)) playerCounts[sp.session_id] = (playerCounts[sp.session_id] || 0) + 1;
      });
      ...
      const openNewSession = urlParams.get("new") === "1";
      if (openNewSession) {
        setSelectedSession(null);
        setView("new");
      } else if (sessionId) {
  ...
  async function handleDelete(id) {
    ...
    if (target.status === "cancelled") {
      toast({ title: "La sesión ya está cancelada" });
      return;
    }
    if (!window.confirm("¿Cancelar esta sesión? Los jugadores, GPS, fuerza, campo y videos se conservarán para auditoría.")) return;
    const updated = await base44.entities.TrainingSession.update(id, { status: "cancelled" });
    setSessions(prev => prev.map(s => s.id === id ? { ...s, ...updated } : s));
    toast({ title: "Sesión cancelada", description: "No se eliminó ningún dato ni ejercicio de biblioteca." });
  }
  ...
  const filteredSessions = useMemo(() => {
    ...
    if (f.minPlayers) list = list.filter(s => (playerCounts[s.id] || 0) >= parseInt(f.minPlayers));
    ...
    else if (f.sort === "jugadores") list.sort((a, b) => (playerCounts[b.id] || 0) - (playerCounts[a.id] || 0));
    else list.sort((a, b) => b.date.localeCompare(a.date));

    return list;
  }, [sessions, filters, weeklyPlans, weeklyPlanDays, playerCounts]);
  ...
  const tourSteps = view === "detail" ? SESSIONS_DETAIL_TOUR : view === "new" ? SESSIONS_CREATE_TOUR : SESSIONS_LIST_TOUR;
  const handleTourStep = useCallback((step) => {
    if (view === "detail" && step.tab) setSelectedTab(step.tab);
  }, [view]);

  if (demoActive) return <DemoSessions />;

  return (
    <div className="space-y-6" data-tour="sessions-page">
      {/* Header */}
      {view === "list" && (
        <div data-tour="sessions-header" className="flex items-center justify-between gap-3 flex-wrap">
      ...
      {view === "new" && (
        <>
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-0.5">Configurá el estado de cada jugador directamente en la sesión</p>
          </div>
      ...
      {view === "list" && (
        loading ? (
          ...
          <>
            <div data-tour="sessions-filters"><SessionFilters filters={filters} onChange={setFilters} physicalObjectives={physicalObjectives} /></div>
            <div data-tour="sessions-list"><SessionList sessions={filteredSessions} onSelect={handleSelect} onDelete={handleDelete} hasFilters={hasActiveFilters} exerciseCounts={exerciseCounts} playerCounts={playerCounts} videoLinksBySession={videoLinksBySession} weeklyPlans={weeklyPlans} physicalObjectives={physicalObjectives} /></div>
          </>
        )
      )}
