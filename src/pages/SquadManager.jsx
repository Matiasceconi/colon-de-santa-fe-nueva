...
// Convierte un nombre guardado en mayúsculas a formato título (Jeremías Chazarreta).
...
// ─── Squad Form ────────────────────────────────────────────────────────────
function SquadForm({ squad, onSave, onCancel, seasons = [], categories = [] }) {
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
            className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-zinc-500">
            <option value="">Sin categoría adicional</option>
            {[...new Set([...categories, form.category].filter(Boolean))].map((category) => <option key={category} value={category}>{category}</option>)}
          </select>
        </div>
        <div>
          ...
          <select value={form.season} onChange={e => setForm(f => ({ ...f, season: e.target.value }))}
            ...
            {[...new Set([...seasons, form.season].filter(Boolean))].map((season) => <option key={season} value={season}>{season}</option>)}
          </select>
        </div>
        <div>
...
function AddPlayerModal({ squad, players, existingMemberships, onSave, onClose }) {
  ...
  // Una sola membresía "activo" vigente define el plantel canónico. Si el jugador ya
  // está activo en otro plantel debe moverse desde ese plantel, no duplicarse desde acá.
  const currentMemberships = existingMemberships.filter(m => m.status === "activo" && !m.effective_to);
  const activePlayerIds = new Set(
    currentMemberships.filter(m => m.squad_id === squad.id).map(m => m.player_id)
  );
  const activeElsewhereByPlayer = new Map(
    currentMemberships.filter(m => m.squad_id !== squad.id).map(m => [m.player_id, m])
  );
  ...
  const filtered = players.filter(p => {
    if (activePlayerIds.has(p.id)) return false; // ya está en este plantel
    if (activeElsewhereByPlayer.has(p.id)) return false; // debe usar "Mover jugador" desde su plantel actual
      ...
      <div className="bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl w-full max-w-xl flex flex-col" style={{ maxHeight: "90vh" }}>
        ...
        <div className="flex items-center justify-between p-5 border-b border-zinc-800 shrink-0">
          <div>
            ...
            <p className="mt-1 text-[10px] text-zinc-600">Los jugadores con otro plantel activo se mueven desde su plantel actual para conservar el historial.</p>
          </div>
              ...
              return (
                <button key={p.id} onClick={() => toggle(p.id)}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white font-medium truncate">{toTitleName(p.full_name)}</p>
...
// ─── Squad Detail Card ─────────────────────────────────────────────────────
function SquadCard({ squad, memberships, players, onEditSquad, onDeleteSquad, onAddPlayer, onMovePlayer, onRemoveFromSquad }) {
  const [expanded, setExpanded] = useState(false);
  const activeMembers = memberships.filter(m => m.status === "activo" && !m.effective_to);
              ...
              {memberships.map(m => {
                ...
                return (
                  <div key={m.id} className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-zinc-800/50">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-white truncate">{toTitleName(m.player_name)}</p>
...
export default function SquadManager() {
  ...
  const { seasons, categories } = useInstitutionStructureOptions();
  ...
  async function load() {
    ...
    try {
      ...
      const sq = await base44.entities.Squad.list("name", 100);
      const mb = await base44.entities.SquadMembership.list("-effective_from", 1000);
      const pl = await base44.entities.Player.list("-created_date", 500);
      setSquads(sq);
      setMemberships(mb);
      setPlayers(pl.filter(p => p.active !== false));
    } catch (error) {
      console.error("SquadManager load error:", error);
      toast({ title: "No se pudo cargar la gestión de planteles", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSaveSquad(data) {
    const clean = { ...data, name: String(data.name || "").trim(), category: String(data.category || "").trim(), season: String(data.season || "").trim(), staff: String(data.staff || "").trim(), notes: String(data.notes || "").trim() };
    if (editingSquad) {
      const oldName = editingSquad.name || "";
      await base44.entities.Squad.update(editingSquad.id, clean);
      if (clean.name && clean.name !== oldName) {
        const related = memberships.filter((membership) => membership.squad_id === editingSquad.id);
        await Promise.all(related.map((membership) => base44.entities.SquadMembership.update(membership.id, { squad_name: clean.name })));
        const activePlayerIds = [...new Set(related.filter((membership) => membership.status === "activo" && !membership.effective_to).map((membership) => membership.player_id).filter(Boolean))];
        await Promise.all(activePlayerIds.map((playerId) => base44.entities.Player.update(playerId, { squad_id: editingSquad.id, squad_name: clean.name, division: clean.name })));
      }
      await load();
      toast({ title: "Plantel actualizado", description: clean.name !== oldName ? "También se sincronizaron sus vínculos vigentes." : undefined });
    } else {
      const created = await base44.entities.Squad.create(clean);
  ...
  async function handleDeleteSquad(squad) {
    if (!confirm(`¿Desactivar el plantel "${squad.name}"? No se borrarán jugadores ni historial de membresías.`)) return;
    await base44.entities.Squad.update(squad.id, { active: false });
    setSquads(prev => prev.map((row) => row.id === squad.id ? { ...row, active: false } : row));
    toast({ title: "Plantel desactivado", description: "El historial se conserva y puede reactivarse editando el plantel." });
  }

  async function syncPlayerPrimary(data) {
    if (!data?.player_id) return;
    await base44.entities.Player.update(data.player_id, { squad_id: data.squad_id || "", squad_name: data.squad_name || "", division: data.squad_name || "" });
  }
  ...
  async function handleRemoveFromSquad(membership) {
    ...
    const alternate = memberships.find((row) => row.player_id === membership.player_id && row.id !== membership.id && row.status === "activo" && !row.effective_to);
    if (alternate) await syncPlayerPrimary(alternate);
    else await base44.entities.Player.update(membership.player_id, { squad_id: "", squad_name: "", division: "" });
    toast({ title: `${membership.player_name} quitado del plantel`, description: "La membresía quedó en el historial; no se eliminó." });
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">{squads.filter((s) => s.active !== false).length} planteles activos · {memberships.filter(m => m.status === "activo" && !m.effective_to).length} membresías vigentes</p>
        </div>
      ...
      {squads.length === 0 ? (
        ...
        <div className="space-y-3">
          {squads.map(squad => {
            const squadMemberships = memberships.filter(m => m.squad_id === squad.id && m.status === "activo" && !m.effective_to);
            return (
              <SquadCard
                key={squad.id}
                squad={squad}
